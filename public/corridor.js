...
const S = {
  ...
  strokeNumbers: false,
  /** The hosted review can enter 書の間 as a true edge-to-edge room. This is
   * URL/session presentation state, never learner evidence. */
  strokeMinimal: false,
  /** Minimal 書の間 sleeps every control field until its single faint wake
   * trigger is pressed. The framed room keeps its established open chrome. */
  strokeChromeAwake: true,
...
async function boot() {
  ...
  S.strokeMinimal = params.get('minimal') === '1';
  ...
  render();
  // First paint is always the deterministic paper. The live ground mounts
  // afterward without holding corpus/data boot or the first readable frame.
  void mountInkSubstrate();
  ...
  // Hosted design-review entry: open the exact 書の間 specimen first,
  // while keeping the full corridor one Back tap away.
  if (params.get('demo') === 'ink') {
    S.strokes = { id: '永', step: 0, sheetScroll: 0, invoker: null };
  }
  render();
  prefetchArticles();
}
...
/** One source of truth for the numbered overlay. The page remembers how many
 * strokes have actually begun even while the learner has numbers switched
 * off; switching them on never invents a future number. */
function syncStrokeNumbers(page, shown = Number(page.dataset.strokeShown || 0)) {
  const markers = [...page.querySelectorAll('.stroke-num')];
...
function paintStrokes(page, shown, progress = 1) {
  ...
  // Once living ink owns the stage its callbacks own the number clock too;
  // the classic SVG rAF may still be finishing underneath and must not race it.
  if (page.dataset.living !== 'on') syncStrokeNumbers(page, shown);
...
function startStrokeAnimation(page) {
  ...
  // The classic SVG fallback follows the same subtly quicker tempo as the
  // living writer. Reduced motion returned above and remains fully manual.
  const GAP = 83;
  const durations = paths.map((path) =>
    Math.min(920, Math.max(240, (Number(path.dataset.len) || 0) * 8.3)),
  );
  const starts = [];
  let acc = 0;
 ...
 * visible to the learner without reading the punctuation aloud. */
function displayKunReading(reading) {
  const dot = String(reading || '').indexOf('.');
  if (dot < 0) return String(reading || '');
...
/** The one learning option shared by the framed and the quiet room. Turning
 * it on always starts a new demonstration, so each number arrives with the
 * first real ink sample of its stroke. */
function strokeNumbersControl(page, paths, reduced) {
...
/** Minimal mode is an allowlist, not hidden legacy chrome: these are the
 * only things permitted to wake around the glyph. */
function strokeAwakeField(page, k, paths, reduced) {
  const field = el('div', 'stroke-awake-field');
  ...
  for (const id of WRITING_PALETTE_IDS) {
  ...
  const numbers = strokeNumbersControl(page, paths, reduced);
  numbers.classList.add('stroke-number-choice');
  field.append(palettes, readings, numbers);
  return field;
}
 ...
 * of the complete reference worlds carries its own pigment; the sheet's paper is grown
...
const INK_WORLDS = {
  ...
  hokusai: { pal: { mode: 0, low: [0.48, 0.58, 0.78], high: [0.07, 0.14, 0.28], sheen: [0.05, 0.06, 0.08] }, wetScale: 1.0, still: '#1f3766' },
  // 墨 — the canon, stroke-art-v5's exact numbers
  sumi: { pal: { mode: 0, low: [0.512, 0.448, 0.352], high: [0.16, 0.16, 0.24], sheen: [0.05, 0.055, 0.06] }, wetScale: 1.0, still: '#1c1913' },
  // 朱 — shrine vermilion, verbatim from stroke-art-iro
  shu: { pal: { mode: 0, low: [0.95, 0.52, 0.38], high: [0.62, 0.14, 0.1], sheen: [0.06, 0.05, 0.05] }, wetScale: 1.0, still: '#a8281e' },
  // 赤 赤富士の錆 — rust between 朱 and 弁柄 (authored)
  akafuji: { pal: { mode: 0, low: [0.88, 0.55, 0.4], high: [0.55, 0.18, 0.09], sheen: [0.055, 0.05, 0.045] }, wetScale: 1.0, still: '#8c351f' },
  // 柿 焦茶 — the iro card verbatim
  iwa: { pal: { mode: 0, low: [0.7, 0.52, 0.38], high: [0.3, 0.16, 0.08], sheen: [0.05, 0.045, 0.04] }, wetScale: 1.0, still: '#3e2410' },
  // 漆 胡粉 — the iro card verbatim (additive shell-white)
  rokusho: { pal: { mode: 1, low: [0.3, 0.28, 0.25], high: [0.94, 0.91, 0.85], sheen: [0.14, 0.13, 0.12], spark: [1.0, 0.98, 0.92], metal: 0.25 }, wetScale: 0.9, still: '#efe9dc' },
  // 金 金泥 — the canon, kept no matter what
  yoru: { pal: { mode: 1, low: [0.42, 0.3, 0.1], high: [1.0, 0.83, 0.45], sheen: [0.1, 0.12, 0.2], spark: [1.0, 0.9, 0.6], metal: 1 }, wetScale: 0.85, still: '#d9b25f' },
  // 浪 波の泡 — spindrift white on the deep sea (authored)
  nami: { pal: { mode: 1, low: [0.32, 0.38, 0.44], high: [0.9, 0.94, 0.97], sheen: [0.1, 0.12, 0.16], spark: [0.95, 1.0, 1.0], metal: 0.35 }, wetScale: 0.9, still: '#eae6d8' },
  // 板 — canonical keyblock sumi on print cream
  keyblock: { pal: { mode: 0, low: [0.512, 0.448, 0.352], high: [0.16, 0.16, 0.24], sheen: [0.05, 0.055, 0.06] }, wetScale: 1.0, still: '#26221c' },
  // 雷 — canonical gold carried onto 山下白雨's storm paper
  hakuu: { pal: { mode: 1, low: [0.42, 0.3, 0.1], high: [1.0, 0.83, 0.45], sheen: [0.1, 0.12, 0.2], spark: [1.0, 0.9, 0.6], metal: 1 }, wetScale: 0.85, still: '#d9a93f' },
  // 殻 燐光 — phosphor on terminal black (authored)
  kaku: { pal: { mode: 1, low: [0.08, 0.3, 0.24], high: [0.5, 1.0, 0.82], sheen: [0.08, 0.14, 0.12], spark: [0.6, 1.0, 0.85], metal: 0.6 }, wetScale: 0.9, still: '#50f0c3' },
};
let inkModulePromise = null;
function ensureInkModule() {
  if (!inkModulePromise) {
    const embedded = document.querySelector('#corridor-ink-module');
    if (window.__CORRIDOR_STANDALONE__ && embedded) {
      const source = JSON.parse(embedded.textContent);
      const url = URL.createObjectURL(new Blob([source], { type: 'text/javascript' }));
      inkModulePromise = import(url).finally(() => URL.revokeObjectURL(url));
    } else {
      inkModulePromise = import('./corridor-ink.js');
    }
  }
...
const strokeRewriteOptions = () =>
  S.strokeSlow
    ? { speed: 0.7, iterations: 1 }
    : { speed: 1, iterations: 1.1 };
...
function inkGroundFor(world, size) {
  ...
  const S = size;
  const u = S / 920;
  const r = paperRng({ cream: 13, shell: 17, kozo: 11, dawn: 19, kaki: 29, urushi: 41, kon: 23, sea: 31, storm: 43, terminal: 59 }[world.paper] || 7);
  ...
  if (kind === 'cream') {
    ...
    fibers(2200, (r) => `rgba(180,158,116,${0.03 + r() * 0.06})`, (r) => `rgba(252,246,228,${0.08 + r() * 0.09})`);
    ...
    for (let i = 0; i < 22; i++) {
      const x = r() * S, y = r() * S, len = (30 + r() * 120) * u;
      g.beginPath();
      g.moveTo(x, y);
...
const inkRoomPaperCache = new Map();
function inkRoomPaperFor(world) {
  if (inkRoomPaperCache.has(world.id)) return inkRoomPaperCache.get(world.id);
  try {
    const url = `url("${inkGroundFor(world, 760).toDataURL('image/png')}")`;
    inkRoomPaperCache.set(world.id, url);
    return url;
  } catch {
    return 'none';
  }
...
async function mountInkRoom(room2, ch, paths, reduced) {
  stopInkRoom();
  // The writing hero gets the full simulation budget. The retained ground
  // frame remains visible beneath the modal, but no second lattice advances.
  suspendInkSubstrate('hero');
  ...
  try {
    ...
    // Normally the room OPENS on the finished dried sheet: the first write
    // runs unseen at full simulation density. If numbers were already asked
    // for, the first write is visible instead so no completed set can simply
    // appear all at once; every marker must arrive with its own brush stroke.
    spec.first = page.dataset.numbers === 'on'
      ? strokeRewriteOptions()
      : { iterations: 12, hidden: true };
    ...
    let hiddenWrite = false;
    spec.onStroke = (ix) => {
      if (hiddenWrite) return;
    ...
    spec.onPhase = (phase, detail) => {
      if (phase === 'writing') {
        hiddenWrite = !!(detail && detail.hidden);
        fillPips(0);
        syncStrokeNumbers(page, 0);
        if (!hiddenWrite) room.canvas.classList.remove('brewing');
      }
      if (phase === 'done') {
        const completedUnseen = hiddenWrite;
        hiddenWrite = false;
        fillPips(strokes.length);
        syncStrokeNumbers(page, completedUnseen ? 0 : strokes.length);
    ...
    room.handle = handle;
    // A very fast learner can wake chrome and request numbers while the
    // backend is still starting. Honor that latest choice with a visible
    // rewrite instead of letting the already-scheduled hidden brew win.
    if (page.dataset.numbers === 'on' && spec.first?.hidden) {
      handle.rewrite(strokeRewriteOptions());
    }
    publishInkGround('snapshot', { backend: document.documentElement.dataset.inkBackend });
    handle.ondead = () => {
    ...
    stage.addEventListener('click', () => {
      if (inkRoom === room && room.handle) room.handle.rewrite(strokeRewriteOptions());
    });
  } catch (e) {
...
/** Wake or put away the quiet room's single allowed field. Geometry stays in
 * place; inert + aria-hidden keep it out of keyboard and screen-reader
 * navigation while the glyph is alone. */
...
function renderStrokePage(root) {
  ...
  // One actual room, not a full-screen whiteboard. Its proportions and
  // material are the screenshot source's 390×760 specimen card; the page
  // outside it is merely the receded corridor, not copied Safari chrome.
  const roomSurface = el('div', 'stroke-room');
  roomSurface.style.setProperty('--ink-room-url', inkRoomPaperFor(THEME_UI[themeIx()]));
  ...
  bar.append(seal);
  if (!S.strokeMinimal) roomSurface.append(bar);

  if (!paths.length) {
    strokeMissing(roomSurface, st.id, k);
  } else {
    ...
    if (k?.on?.length) readings.append(el('span', 'stroke-on', k.on.join('・')));
    for (const kn of k?.kun || []) {
    ...
    if (S.strokeMinimal) {
      body.append(strokeAwakeField(page, k, paths, reduced));
    } else {
      const controls = el('div', 'stroke-controls');
      controls.id = 'stroke-controls';
      controls.dataset.strokeChrome = '';
      if (reduced) {
      ...
      // The framed room retains its four local diagram inks. The quiet room
      // instead exposes every complete world directly in strokeAwakeField().
      const inks = el('div', 'stroke-inks');
      inks.dataset.strokeChrome = '';
      ...
      controls.append(strokeNumbersControl(page, paths, reduced));
      body.append(controls);

      const meta = el('p', 'stroke-meta');
      meta.dataset.strokeChrome = '';
  ...
  if (S.strokeMinimal) {
    ...
    roomSurface.append(wake);
  }

  page.append(roomSurface);
  if (S.strokeMinimal) setStrokeChrome(page, S.strokeChromeAwake);
  ...
  queueMicrotask(() => (S.strokeMinimal ? page : backBtn).focus({ preventScroll: true }));
}
 ...
 * The locked eight plus the restored 朱・板・雷 references (2026-08-13,
...
const THEME_UI = [
  ...
  { id: 'shu', seal: '朱', name: '朱・胡粉', g: '#f9f5eb', ink: '#2b2620', red: '#b02318', paper: 'shell' },
  ...
  { id: 'keyblock', seal: '板', name: '板木・墨', g: '#f2ead6', ink: '#26221c', red: '#b03a2e', paper: 'cream' },
  // 山下白雨 — lightning-gold on the storm
  { id: 'hakuu', seal: '雷', name: '山下白雨', g: '#221b12', ink: '#efe5cf', red: '#e0796b', paper: 'storm' },
  // 攻殻・燐光 — phosphor on terminal black
  { id: 'kaku', seal: '殻', name: '攻殻・燐光', g: '#050d12', ink: '#d8efe9', red: '#ff6b4a', paper: 'terminal' },
];
// The writing-room strip is the exact ten-seal reference order. 殻 remains a
// complete app world for saved preferences and the wider corridor, but it is
// not substituted for 朱・板・雷 in this room again.
const WRITING_PALETTE_IDS = [
  'sumi',
  'shu',
  'iwa',
  'rokusho',
...
function commitKairoTheme(id) {
  ...
  document.documentElement.dataset.inkWorld = t.id;
  delete document.documentElement.dataset.inkPendingWorld;
  const night = ['rokusho', 'yoru', 'nami', 'hakuu', 'kaku'].includes(t.id);
  document.documentElement.style.colorScheme = night ? 'dark' : 'light';
  document.querySelector('meta[name="color-scheme"]')?.setAttribute('content', night ? 'dark' : 'light');
  try {
...
/** Ask the living ground to carry one world into the next before the DOM
 * tokens move. The latest stone wins: a second choice invalidates the first
 * flood rather than queueing several seconds of stale colour. Static and
 * reduced-motion rooms commit immediately. */
let themeRequestToken = 0;
async function requestKairoTheme(id) {
  const t = THEME_UI.find((x) => x.id === id) || THEME_UI[0];
  if (t.id === themeId() && !pendingThemeId) return;
  pendingThemeId = t.id;
  document.documentElement.dataset.inkPendingWorld = t.id;
  const token = ++themeRequestToken;

  // 書の間 owns the live engine while it is visible. Put that writer down,
  // wake the ground for the wash, then render a freshly inked room after the
  // destination settles. This keeps the two-engine ceiling honest.
  if (S.strokes) stopInkRoom();

  let groundReady = false;
  try {
    if (inkSubstrate?.handle && !strokeReduced()) {
      publishInkGround('waking', {
        backend: document.documentElement.dataset.inkBackend,
        world: themeId(),
        pendingWorld: t.id,
      });
      await inkSubstrate.handle.wake?.();
      publishInkGround('flood', {
        backend: document.documentElement.dataset.inkBackend,
        world: themeId(),
        pendingWorld: t.id,
      });
      const receipt = await inkSubstrate.handle.flood(substrateSpecFor(t));
      if (receipt?.status === 'failed') throw new Error(receipt.error || 'world flood failed');
      groundReady = !!inkSubstrate.handle.state?.().awake;
    }
  } catch (err) {
    console.warn('ink ground: world wash fell back to a clean cut —', err && err.message);
  }
  if (token !== themeRequestToken) return;
  commitKairoTheme(t.id);
  render();
  if (groundReady && inkSubstrate?.handle && !S.strokes) {
    publishInkGround('live', { backend: document.documentElement.dataset.inkBackend, world: t.id });
    scheduleInkPace();
  } else if (inkSubstrate?.handle?.state?.().phase === 'dead') {
    publishInkGround('static', { backend: 'static', world: t.id });
  }
  window.dispatchEvent(new CustomEvent('kairo:ink-settled', { detail: { world: t.id } }));
}

/** Boot and non-animated callers use a direct commit. */
function setKairoTheme(id) {
  commitKairoTheme(id);
}
/** Drift owns five atmospheric families, while the corridor owns concrete
 * paper worlds. Map by identity so adding or reordering paper worlds never
 * sends an unrelated raw array index into Drift. */
...
// (the old tap-to-cycle retired 2026-08-14 — with many worlds the operator
...
const PAPER_SEED = { cream: 13, shell: 17, kozo: 11, dawn: 19, kaki: 29, urushi: 41, kon: 23, sea: 31, storm: 43, terminal: 59 };
...
function paintPaper(g, w, h, world, base) {
  ...
  if (world.paper === 'cream') {
    ...
    fibers(2150, (r) => `rgba(170,150,110,${0.02 + r() * 0.035})`, (r) => `rgba(252,248,232,${0.06 + r() * 0.07})`, 0.5);
    vignette('rgba(85,70,42,0.032)');
  } else if (world.paper === 'shell') {
    // 朱 shell white — smooth gofun paper with pale shell fibers
    wash([[0, 'rgba(255,253,246,0.035)'], [1, 'rgba(80,70,55,0.012)']], 0.9);
    fibers(2300, (r) => `rgba(190,175,150,${0.016 + r() * 0.03})`, (r) => `rgba(255,252,244,${0.05 + r() * 0.055})`, 0.5);
    ...
    fibers(2550, (r) => `rgba(180,158,116,${0.02 + r() * 0.035})`, (r) => `rgba(252,246,228,${0.06 + r() * 0.07})`, 0.5);
    ...
    fibers(1900, (r) => `rgba(180,140,100,${0.02 + r() * 0.03})`, (r) => `rgba(255,246,230,${0.05 + r() * 0.06})`, 0.5);
    ...
    fibers(1320, (r) => `rgba(120,70,30,${0.02 + r() * 0.03})`, (r) => `rgba(240,200,150,${0.04 + r() * 0.045})`, 0.5);
    ...
    wash([[0, 'rgba(90,70,40,0.025)'], [1, 'rgba(0,0,0,0.045)']], 0.4);
    for (let i = 0; i < 22; i++) {
      const x = r() * w;
      const y = r() * h;
...
/* ---------------------------------------------------- Z2 live ink substrate
 * The readable corridor keeps real DOM text and the quiet paper painter above.
 * Beneath it, the SAME fluid module as 書の間 runs one 512-square lattice:
 * true D2Q9 on WebGPU, the module's explicitly-labelled fluid approximation
 * on WebGL2, and the deterministic paper above as the honest static fallback.
 * The canvas is square and CSS-cropped (never stretched), so a phone and a
 * landscape window read the same fibre/flow proportions. */
let inkSubstrate = null; // { canvas, snapshot, handle }
let inkSubstrateMount = null;
let inkPaceSlowTimer = 0;
let inkPaceSleepTimer = 0;

function substrateGroundFor(world, size) {
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = size;
  const g = canvas.getContext('2d');
  if (g) paintPaper(g, size, size, world, world.g);
  return canvas;
}

function substrateSpecFor(world) {
  const iw = INK_WORLDS[world.id] || INK_WORLDS.sumi;
  return {
    pal: { ...iw.pal, inkScale: 0.032 },
    wetScale: 0.22,
    ground: (_r, size) => substrateGroundFor(world, size),
    gpuN: 512,
    gl2Sim: 512,
    seed: PAPER_SEED[world.paper] || 7,
    world: world.id,
  };
}

function publishInkGround(phase, detail = {}) {
  const root = document.documentElement;
  root.dataset.inkGround = phase;
  if (detail.backend) root.dataset.inkBackend = detail.backend;
  root.dataset.inkLiveCount = String((inkRoom?.handle ? 1 : 0) + (phase === 'live' || phase === 'flood' ? 1 : 0));
  window.dispatchEvent(new CustomEvent('kairo:ink-state', { detail: { phase, ...detail } }));
}

function captureInkGroundSnapshot() {
  const room = inkSubstrate;
  if (!room?.canvas || !room.snapshot) return false;
  try {
    room.snapshot.width = room.canvas.width;
    room.snapshot.height = room.canvas.height;
    const g = room.snapshot.getContext('2d');
    if (!g) return false;
    g.drawImage(room.canvas, 0, 0);
    room.snapshot.dataset.ready = '1';
    document.documentElement.dataset.inkSnapshot = 'ready';
    return true;
  } catch {
    delete document.documentElement.dataset.inkSnapshot;
    return false;
  }
}

function clearInkPaceTimers() {
  clearTimeout(inkPaceSlowTimer);
  clearTimeout(inkPaceSleepTimer);
  inkPaceSlowTimer = 0;
  inkPaceSleepTimer = 0;
}

function scheduleInkPace() {
  if (!inkSubstrate?.handle || document.hidden || S.strokes) return;
  clearInkPaceTimers();
  inkSubstrate.handle.setPace?.(30);
  inkPaceSlowTimer = setTimeout(() => inkSubstrate?.handle?.setPace?.(12), 10_000);
  inkPaceSleepTimer = setTimeout(() => void sleepInkSubstrate('idle'), 30_000);
}

async function sleepInkSubstrate(reason = 'idle') {
  if (!inkSubstrate?.handle) return;
  clearInkPaceTimers();
  const captured = captureInkGroundSnapshot();
  await inkSubstrate.handle.sleep?.(reason);
  publishInkGround(captured ? 'snapshot' : 'static', {
    backend: document.documentElement.dataset.inkBackend,
    reason,
  });
}

function inkGroundActivity(event) {
  const room = inkSubstrate;
  if (
    !room?.handle ||
    event.target?.closest?.('#ink-stage') ||
    (S.view === 'drift' && !S.stack.length)
  )
    return;
  ...
  const inject = () => {
    if (inkSubstrate !== room) return;
    room.handle.impulse?.(x, y, Math.max(0.12, event.pressure || 0.2));
    scheduleInkPace();
  };
  if (room.handle.state?.().awake) inject();
  else void resumeInkSubstrate().then(inject);
}

async function mountInkSubstrate() {
  if (inkSubstrateMount) return inkSubstrateMount;
  try {
    if (matchMedia('(prefers-reduced-motion: reduce)').matches) {
      publishInkGround('static', { backend: 'static' });
      return null;
    }
  } catch {
    /* old browser — attempt the live path */
  }
  inkSubstrateMount = (async () => {
    const app = document.querySelector('#app');
    if (!app) return null;
    const canvas = el('canvas', 'ink-substrate-live');
    canvas.id = 'ink-substrate';
    canvas.setAttribute('aria-hidden', 'true');
    const snapshot = el('canvas', 'ink-substrate-snapshot');
    snapshot.id = 'ink-substrate-snapshot';
    snapshot.setAttribute('aria-hidden', 'true');
    app.before(snapshot, canvas);
    const room = (inkSubstrate = { canvas, snapshot, handle: null });
    const world = THEME_UI[themeIx()];
    const spec = substrateSpecFor(world);
    spec.freshCanvas = () => {
      const fresh = room.canvas.cloneNode(false);
      room.canvas.replaceWith(fresh);
      room.canvas = fresh;
      return fresh;
    };
    publishInkGround('waking', { world: world.id });
    try {
      const INK = await ensureInkModule();
      if (inkSubstrate !== room || !document.contains(room.canvas)) return null;
      const handle = await INK.startSubstrate(room.canvas, spec);
      if (inkSubstrate !== room) {
        handle.stop();
        return null;
      }
      room.handle = handle;
      const backend = handle.kind === 'gpu' ? 'webgpu-d2q9' : 'webgl2-fluid';
      publishInkGround('live', { backend, world: world.id });
      scheduleInkPace();
      return room;
    } catch (err) {
      console.warn('ink ground unavailable — static washi remains:', err && err.message);
      if (inkSubstrate === room) inkSubstrate = null;
      room.canvas.remove();
      room.snapshot.remove();
      publishInkGround('static', { backend: 'static', world: world.id });
      return null;
    }
  })();

  window.addEventListener('pointerdown', inkGroundActivity, { passive: true, capture: true });
  window.addEventListener('pointermove', (event) => {
    if (event.buttons || event.pointerType === 'touch') inkGroundActivity(event);
  }, { passive: true, capture: true });
  window.addEventListener('scroll', scheduleInkPace, { passive: true });
  document.addEventListener('visibilitychange', () => {
    if (!inkSubstrate?.handle) return;
    if (document.hidden) {
      void sleepInkSubstrate('hidden');
    } else {
      resumeInkSubstrate();
    }
  });
  window.addEventListener('pagehide', () => void sleepInkSubstrate('pagehide'));
  window.addEventListener('pageshow', resumeInkSubstrate);
  return inkSubstrateMount;
}

function suspendInkSubstrate(reason = 'hero') {
  if (!inkSubstrate?.handle) return;
  void sleepInkSubstrate(reason);
}

function resumeInkSubstrate() {
  if (
    !inkSubstrate?.handle ||
    document.hidden ||
    S.strokes ||
  ...
  const room = inkSubstrate;
  if (room.handle.state?.().awake) {
    if (document.documentElement.dataset.inkGround !== 'live')
      publishInkGround('live', { backend: document.documentElement.dataset.inkBackend, world: themeId() });
    scheduleInkPace();
    return Promise.resolve(room.handle.state?.() || null);
  }
  publishInkGround('waking', { backend: document.documentElement.dataset.inkBackend, world: themeId() });
  return Promise.resolve(room.handle.wake?.())
    .then(() => {
      if (inkSubstrate !== room || document.hidden || S.strokes) return null;
      publishInkGround('live', { backend: document.documentElement.dataset.inkBackend, world: themeId() });
      scheduleInkPace();
      setTimeout(() => {
        if (inkSubstrate !== room || document.documentElement.dataset.inkGround !== 'live') return;
        room.snapshot.removeAttribute('data-ready');
        delete document.documentElement.dataset.inkSnapshot;
      }, 320);
      return room.handle.state?.() || null;
    })
    .catch((err) => {
      console.warn('ink ground could not wake —', err && err.message);
      publishInkGround('static', { backend: 'static', world: themeId() });
      return null;
    });
}

window.__KAIRO_INK__ = Object.freeze({
  state() {
    return Object.freeze({
      backend: document.documentElement.dataset.inkBackend || 'static',
      phase: document.documentElement.dataset.inkGround || 'static',
      world: themeId(),
      pendingWorld: pendingThemeId || null,
      liveCount: Number(document.documentElement.dataset.inkLiveCount || 0),
      engine: inkSubstrate?.handle?.state?.() || null,
    });
  },
});
 ...
 * 2026-08-14): tapping a theme seal opens the stones for a direct
...
function render() {
  ...
  if (window.__DRIFT__) {
    syncDriftTheme();
    if (S.view === 'drift' && !S.stack.length) {
      suspendInkSubstrate('drift');
