...
export function makeWriterFor(strokes) {
  ...
  return {
    ...
    advance(now) {
      ...
      if (raw >= 0 && prevT < 1) {
        // This signal is the first real ink sample—not the anticipatory pause.
        // Number overlays can therefore rise in the same frame as the brush.
...
export function createLBM(device, N, pal) {
  ...
  function writeParams(nSplats, seed) {
    ...
    pf[8] = 0.97; pf[9] = 0.0022; pf[10] = palette.inkScale ?? 1.0; pf[11] = palette.metal || 0;
...
function startGPU(canvas, spec, device) {
  ...
  let mode = 'idle', writeSpeed = 1, iterations = 1, iterationCarry = 0, writeClock = 0, dryLeft = 0;
  const DRY_STEPS = 240;
  const takeIterations = (scale = 1) => {
    iterationCarry += iterations * scale;
    const count = Math.max(1, Math.floor(iterationCarry));
    iterationCarry -= count;
    return count;
  };
  function begin(opts) {
    try {
  ...
  function frame(now) {
    ...
    try {
      if (mode === 'writing') {
        const stepsThisFrame = takeIterations();
        for (let k = 0; k < stepsThisFrame && mode === 'writing'; k++) {
...
uniform int uMode;
uniform float uPx, uMetal, uInkScale;
...
void main(){
  ...
  ink = max(ink + 0.95 * (max(st.b, 0.) - blur), 0.) * uInkScale;
...
function startGL2(canvas, spec) {
  ...
  function render() {
    ...
    gl.uniform1f(U(pRender, 'uInkScale'), spec.pal.inkScale ?? 1);
...
/* ======================================================== app substrate ===
 * The hero above owns a writer. The app ground owns no glyph: it keeps the
 * same lattice moving under a full-alpha mask, accepting sparse weather,
 * touch impulses and a bounded world-change flood. The app shell owns the
 * snapshot DOM and the MAX_LIVE arbitration; this module owns only the live
 * surface and never claims a second GPUDevice.
 *
 * WebGPU is the true D2Q9 solver (`createLBM`, fixed at 512² here). WebGL2 is
 * deliberately reported as `fluid`, not D2Q9: it reuses the honest half-float
 * diffusion/deposition fallback used by the hero.
 */
export const MAX_LIVE = 2;
const SUBSTRATE_GPU_N = 512;
const SUBSTRATE_GL2_N = 384;
const SUBSTRATE_FPS = 30;
const SUBSTRATE_FLOOD_MS = 1300;
const SUBSTRATE_FLOOD_DEADLINE_MS = 1650;

const clamp01 = (v) => Math.max(0, Math.min(1, Number(v) || 0));

function substrateSpec(next, previous = null) {
  const merged = { ...(previous || {}), ...(next || {}) };
  const oldPal = (previous && previous.pal) || {};
  const nextPal = (next && next.pal) || {};
  const pal = { ...oldPal, ...nextPal };
  pal.mode ??= 0;
  pal.low ||= [0.52, 0.48, 0.42];
  pal.high ||= [0.18, 0.18, 0.22];
  pal.sheen ||= [0.02, 0.02, 0.025];
  // A hero uses 1.0. The ground is deliberately quiet so DOM text keeps its
  // measured contrast; callers may tune this downward per world.
  pal.inkScale = next?.inkScale ?? nextPal.inkScale ?? oldPal.inkScale ?? 0.035;
  return {
    ...merged,
    id: merged.id || merged.world || 'sumi',
    pal,
    wetScale: merged.wetScale ?? 1,
  };
}

function substrateGroundSource(spec, size, seed) {
  if (typeof spec.ground === 'function') return spec.ground(rng(seed), size);
  if (spec.ground && typeof spec.ground !== 'string') return spec.ground;
  const c = document.createElement('canvas');
  c.width = c.height = size;
  const g = c.getContext('2d');
  g.fillStyle = typeof spec.ground === 'string' ? spec.ground : (spec.groundColor || '#f5efe0');
  g.fillRect(0, 0, size, size);
  return c;
}

function substrateSolidMask(size) {
  const c = document.createElement('canvas');
  c.width = c.height = size;
  const g = c.getContext('2d');
  g.fillStyle = '#fff';
...
function substrateSplat(xNorm, yNorm, pressure, spec) {
  const p = Math.max(0.08, clamp01(pressure));
  const radius = spec.impulseRadius ?? 58;
  return {
    x: clamp01(xNorm) * 1024,
    y: clamp01(yNorm) * 1024,
    rCore: radius * (0.75 + p * 0.45),
    rWater: radius * (1.35 + p * 0.7),
    wet: (spec.impulseWet ?? 0.018) * p * (spec.wetScale ?? 1),
    dry: (spec.impulsePigment ?? 0.012) * p,
    press: 0.025 * p,
  };
}

function defaultSubstrateWeather(random, frameNo, spec) {
  // Leave a real >=2s steady-state window between beats for the amplitude gate.
  const every = Math.max(24, Math.round((spec.weatherEveryMs ?? 6000) / (1000 / SUBSTRATE_FPS)));
  ...
  return [{
    ...
    rCore: 34 + random() * 28,
    rWater: 74 + random() * 52,
    wet: (spec.weatherWet ?? 0.006) * (spec.wetScale ?? 1),
    dry: spec.weatherPigment ?? 0.003,
    press: 0.01,
  }];
}

function substrateFloodSplats(t, frameNo, spec) {
  // Five broad, overlapping lanes make one wash rather than a polka-dot field.
  // The transition is exempt from the quiet amplitude law; finalizeFlood()
  // resets onto the destination's clean, measured ground before it resolves.
  if (frameNo % 2) return [];
  const lane = frameNo % 5;
  const forward = lane % 2 === 0;
  const x = forward ? (-0.12 + t * 1.24) : (1.12 - t * 1.24);
  return [{
    x: clamp01(x) * 1024,
    y: (0.1 + lane * 0.2) * 1024,
    rCore: spec.floodRadius ?? 175,
    rWater: spec.floodWaterRadius ?? 245,
    wet: (spec.floodWet ?? 0.026) * (spec.wetScale ?? 1),
    dry: spec.floodPigment ?? 0.03,
    press: 0.035,
  }];
}

/** Shared state machine for both presentation backends.
 * Backend contract: { kind, solver, N, reset(spec,seed), setPalette(pal),
 * frame(splats,seed), release(), close() }. */
function makeSubstrateController(backend, initialSpec) {
  let active = substrateSpec(initialSpec);
  let seed = active.seed == null ? ((Math.random() * 1e9) >>> 0) : (active.seed >>> 0);
  let random = rng(seed ^ 0x9e3779b9);
  let awake = false;
  let stopped = false;
  let phase = 'boot';
  let rafId = 0;
  let lastFrameAt = 0;
  let frameNo = 0;
  let targetFps = Math.max(1, Math.min(SUBSTRATE_FPS, active.fps || SUBSTRATE_FPS));
  const impulses = [];
  let flood = null;
  let floodSeq = 0;
  ...
  const snapshot = () => Object.freeze({
    ...
    stopped,
    world: active.id,
    floodingTo: flood ? flood.next.id : null,
    fps: awake ? targetFps : 0,
    frames: frameNo,
  });

  function notify() {
    try { initialSpec.onState?.(snapshot()); } catch { /* an observer cannot stop ink */ }
  }

  function die(error) {
    if (stopped || phase === 'dead') return;
    awake = false;
    phase = 'dead';
    if (rafId) cancelAnimationFrame(rafId);
    rafId = 0;
    if (flood) {
      clearTimeout(flood.timer);
      flood.resolve({ status: 'failed', error: String(error?.message || error) });
      flood = null;
    }
    notify();
    try { handle.ondead?.(error); } catch { /* observer */ }
  }

  function schedule() {
    if (awake && !stopped && !rafId) rafId = requestAnimationFrame(tick);
  }

  function finishFlood(seq, status = 'settled') {
    if (!flood || flood.seq !== seq) return;
    const pending = flood;
    flood = null;
    clearTimeout(pending.timer);
    active = pending.next;
    seed = active.seed == null ? ((Math.random() * 1e9) >>> 0) : (active.seed >>> 0);
    random = rng(seed ^ 0x9e3779b9);
    targetFps = Math.max(1, Math.min(SUBSTRATE_FPS, active.fps || SUBSTRATE_FPS));
    try {
      backend.setPalette(active.pal);
      // Reset is the settle boundary: no transition pigment is allowed to sit
      // under a text column after the <=1.7s flood promise resolves.
      backend.reset(active, seed);
      phase = awake ? 'live' : 'sleep';
      pending.resolve({ status, world: active.id, elapsed: performance.now() - pending.startedAt });
      notify();
    } catch (error) {
      pending.resolve({ status: 'failed', error: String(error?.message || error) });
      die(error);
    }
  }
  ...
  function tick(now) {
    rafId = 0;
    if (!awake || stopped) return;
    schedule();
    const period = 1000 / targetFps;
    if (lastFrameAt && now - lastFrameAt < period - 1) return;
    lastFrameAt = now;
    try {
      let splats = impulses.splice(0, MAX_SPLATS);
      if (flood) {
        const elapsed = now - flood.startedAt;
        const t = Math.min(1, elapsed / flood.duration);
        splats = splats.concat(substrateFloodSplats(t, frameNo, flood.next));
        if (elapsed >= flood.duration) {
          finishFlood(flood.seq);
          frameNo += 1;
          return;
        }
      } else {
        const weather = typeof active.weather === 'function'
          ? active.weather({ now, frame: frameNo, random, maxSplats: MAX_SPLATS })
          : defaultSubstrateWeather(random, frameNo, active);
        if (Array.isArray(weather)) splats = splats.concat(weather.slice(0, MAX_SPLATS));
      }
      backend.frame(splats.slice(0, MAX_SPLATS), seed);
      frameNo += 1;
    } catch (error) {
      console.error('ink substrate frame failed:', error);
      die(error);
    }
  }
  ...
  const handle = {
    kind: backend.kind,
    ondead: null,
    impulse(xNorm, yNorm, pressure = 0.45) {
      if (stopped) return false;
      impulses.push(substrateSplat(xNorm, yNorm, pressure, active));
      if (impulses.length > MAX_SPLATS) impulses.splice(0, impulses.length - MAX_SPLATS);
      if (!awake) handle.wake().catch(die);
      return true;
    },
    async flood(nextSpec) {
      if (stopped) throw new Error('substrate stopped');
      if (!awake) await handle.wake();
      if (flood) {
        clearTimeout(flood.timer);
        flood.resolve({ status: 'superseded', world: flood.next.id });
        flood = null;
      }
      const next = substrateSpec(nextSpec, active);
      const duration = Math.max(180, Math.min(1400, next.floodMs ?? SUBSTRATE_FLOOD_MS));
      const seq = ++floodSeq;
      backend.setPalette({
        ...next.pal,
        inkScale: next.floodInkScale ?? Math.max(next.pal.inkScale, 0.055),
      });
      phase = 'flood';
      notify();
      return new Promise((resolve) => {
        const startedAt = performance.now();
        // Settle work includes repainting/uploading a 512² ground. Leave 150ms
        // of the public 1.7s budget for that synchronous boundary.
        const hardAfter = Math.min(SUBSTRATE_FLOOD_DEADLINE_MS - 100, duration + 100);
        const timer = setTimeout(() => finishFlood(seq, 'deadline-settled'), hardAfter);
        flood = { seq, next, duration, startedAt, timer, resolve };
        schedule();
      });
    },
    async sleep() {
      if (stopped || !awake) return snapshot();
      awake = false;
      if (rafId) cancelAnimationFrame(rafId);
      rafId = 0;
      lastFrameAt = 0;
      // A flood promise may never be stranded by a page hide. Its destination
      // becomes the next wake's clean world; the app shell swaps tokens/static
      // paper from the resolved receipt.
      if (flood) {
        const pending = flood;
        flood = null;
        clearTimeout(pending.timer);
        active = pending.next;
        pending.resolve({ status: 'sleep-settled', world: active.id, elapsed: performance.now() - pending.startedAt });
      }
      backend.release();
      phase = 'sleep';
      notify();
    ...
    setPace(fps) {
      targetFps = Math.max(1, Math.min(SUBSTRATE_FPS, Number(fps) || SUBSTRATE_FPS));
      notify();
      return targetFps;
    },
    stop() {
      ...
      if (flood) {
        clearTimeout(flood.timer);
        flood.resolve({ status: 'stopped', world: flood.next.id });
        flood = null;
      }
      impulses.length = 0;
      try { backend.close(); } catch { /* already gone */ }
      phase = 'stopped';
      notify();
    },
    state: snapshot,
...
function createGPUSubstrateBackend(canvas, device) {
  const N = SUBSTRATE_GPU_N;
  canvas.width = canvas.height = N;
  const ctx = canvas.getContext('webgpu');
  ...
  const blitMod = device.createShaderModule({ code: WGSL_BLIT, label: 'substrate-blit' });
  const blitPipe = device.createRenderPipeline({
    layout: 'auto',
    vertex: { module: blitMod, entryPoint: 'vs' },
    fragment: { module: blitMod, entryPoint: 'fs', targets: [{ format }] },
    primitive: { topology: 'triangle-list' },
    label: 'substrate-blit',
  });
  const blitSampler = device.createSampler({ magFilter: 'linear', minFilter: 'linear' });
  const mask = substrateSolidMask(N);
  let engine = null;
  let blitBG = null;
  let currentPal = null;
  ...
  function bindFrame() {
    blitBG = device.createBindGroup({
      ...
      label: 'substrate-blit-bg',
    });
  }

  function ensureEngine(pal) {
    if (engine) return;
    currentPal = pal;
    engine = createLBM(device, N, pal);
    bindFrame();
  }

  function blit() {
    const encoder = device.createCommandEncoder({ label: 'substrate-present' });
  ...
  return {
    ...
    setPalette(pal) {
      currentPal = pal;
      if (engine) engine.setPalette(pal);
    },
    reset(spec, seed) {
      ensureEngine(spec.pal);
      currentPal = spec.pal;
      engine.setPalette(spec.pal);
      engine.reset(seed);
      const ground = substrateGroundSource(spec, N, seed + 17);
      device.queue.copyExternalImageToTexture({ source: ground }, { texture: engine.ground }, [N, N]);
      device.queue.copyExternalImageToTexture({ source: mask }, { texture: engine.sprite }, [N, N]);
      engine.frameStep([], seed);
      blit();
    },
    frame(splats, seed) {
      if (!engine) throw new Error('webgpu substrate is asleep');
      engine.frameStep(splats, seed);
      blit();
    },
    release() {
      if (!engine) return;
      try { engine.destroy(); } catch { /* device may already be lost */ }
      engine = null;
      blitBG = null;
    },
    close() {
      this.release();
      try { ctx.unconfigure(); } catch { /* already unconfigured */ }
    },
  };
}

function createGL2SubstrateBackend(canvas, initialSpec) {
  const N = Math.max(256, Math.min(512, Number(initialSpec.gl2Sim) || SUBSTRATE_GL2_N));
  canvas.width = canvas.height = N;
  const gl = canvas.getContext('webgl2', { antialias: false, preserveDrawingBuffer: true });
  if (!gl || !gl.getExtension('EXT_color_buffer_float')) throw new Error('no webgl2 float');
  const linearFloat = !!gl.getExtension('OES_texture_float_linear');
  const stateFilter = linearFloat ? gl.LINEAR : gl.NEAREST;
  const mask = substrateSolidMask(N);
  const quad = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, quad);
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([-1, -1, 3, -1, -1, 3]), gl.STATIC_DRAW);
  ...
  function program(fragmentSource) {
    const p = gl.createProgram();
    for (const [type, source] of [[gl.VERTEX_SHADER, GL2_VS], [gl.FRAGMENT_SHADER, fragmentSource]]) {
  ...
  const pStep = program(GL2_STEP);
  const pSplat = program(GL2_SPLAT);
  const pRender = program(GL2_RENDER);
  ...
  let spriteT = null;
  let cur = 0;
  let currentPal = initialSpec.pal;
  ...
  function texture(internal, format, type, filter) {
    const t = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, t);
  ...
  function allocate() {
    if (state) return;
    state = [0, 1].map(() => texture(gl.RGBA16F, gl.RGBA, gl.HALF_FLOAT, stateFilter));
    fbs = state.map((t) => {
      const f = gl.createFramebuffer();
      gl.bindFramebuffer(gl.FRAMEBUFFER, f);
      gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, t, 0);
      if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE)
        throw new Error('webgl2 substrate framebuffer incomplete');
      return f;
    });
    fiberT = texture(gl.RGBA8, gl.RGBA, gl.UNSIGNED_BYTE, gl.LINEAR);
    groundT = texture(gl.RGBA8, gl.RGBA, gl.UNSIGNED_BYTE, gl.LINEAR);
    spriteT = texture(gl.RGBA8, gl.RGBA, gl.UNSIGNED_BYTE, gl.LINEAR);
  }

  function upload(spec, seed) {
    const ground = substrateGroundSource(spec, N, seed + 17);
    gl.bindTexture(gl.TEXTURE_2D, fiberT);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA8, N, N, 0, gl.RGBA, gl.UNSIGNED_BYTE, buildFiberBytes(seed + 3, N));
    gl.bindTexture(gl.TEXTURE_2D, groundT);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA8, gl.RGBA, gl.UNSIGNED_BYTE, ground);
    gl.bindTexture(gl.TEXTURE_2D, spriteT);
  ...
  function splat(list) {
    ...
    gl.bindTexture(gl.TEXTURE_2D, spriteT);
    gl.uniform1i(U(pSplat, 'uSprite'), 0);
    gl.uniform1f(U(pSplat, 'uSim'), N);
    for (const s of list) {
      gl.uniform2f(U(pSplat, 'uC'), s.x * scale, s.y * scale);
      gl.uniform1f(U(pSplat, 'uRCore'), s.rCore * scale);
      gl.uniform1f(U(pSplat, 'uRWater'), s.rWater * scale);
      gl.uniform1f(U(pSplat, 'uWet'), s.wet);
      gl.uniform1f(U(pSplat, 'uDry'), s.dry);
      draw();
    }
    gl.disable(gl.BLEND);
  ...
  function render() {
    ...
    gl.uniform1i(U(pRender, 'uGround'), 1);
    gl.uniform1i(U(pRender, 'uFiber'), 2);
    gl.uniform1i(U(pRender, 'uMode'), currentPal.mode);
    gl.uniform1f(U(pRender, 'uPx'), 1 / N);
    gl.uniform1f(U(pRender, 'uMetal'), currentPal.metal || 0);
    gl.uniform1f(U(pRender, 'uInkScale'), currentPal.inkScale ?? 1);
    gl.uniform3fv(U(pRender, 'uLow'), currentPal.low);
    gl.uniform3fv(U(pRender, 'uHigh'), currentPal.high);
    gl.uniform3fv(U(pRender, 'uSheen'), currentPal.sheen);
    gl.uniform3fv(U(pRender, 'uSpark'), currentPal.spark || [1, 1, 1]);
    draw();
  }
  ...
  return {
    ...
    frame(splats, _seed) {
      if (!state) throw new Error('webgl2 substrate is asleep');
      if (splats.length) splat(splats);
      step();
      step();
      render();
    },
    release() {
      ...
      for (const t of [...state, fiberT, groundT, spriteT]) gl.deleteTexture(t);
      state = null;
      fbs = null;
      fiberT = groundT = spriteT = null;
    },
    close() {
      this.release();
      gl.deleteProgram(pStep);
      gl.deleteProgram(pSplat);
      gl.deleteProgram(pRender);
...
/** Start the app-wide living ground on a fresh canvas.
 *
 * `spec.weather`, when supplied, receives `{ now, frame, random, maxSplats }`
 * and returns canonical 1024-space splats. `spec.freshCanvas()` is required
 * for a WebGPU→WebGL2 fallback because a claimed canvas context type is
 * claimed forever. The returned handle's flood promise always resolves no
 * later than 1.65s after it begins.
 */
export async function startSubstrate(canvas, spec = {}) {
  const normalized = substrateSpec(spec);
  const device = await acquireDevice();
  if (device) {
    let handle = null;
    device.pushErrorScope('validation');
    try {
      const backend = createGPUSubstrateBackend(canvas, device);
      handle = makeSubstrateController(backend, normalized);
      await handle.wake();
      const validation = await device.popErrorScope();
      if (validation) throw new Error('webgpu validation: ' + validation.message);
      return handle;
    } catch (error) {
      // Balance the scope when setup threw before the explicit pop above.
      try { await device.popErrorScope(); } catch { /* scope was already popped */ }
      try { handle?.stop(); } catch { /* partial handle */ }
      console.warn('ink substrate: webgpu failed, falling back to webgl2 —', error?.message || error);
      if (!spec.freshCanvas) throw new Error('webgpu substrate failed; fallback needs a fresh canvas');
      canvas = spec.freshCanvas();
    }
  }
  const backend = createGL2SubstrateBackend(canvas, normalized);
  const handle = makeSubstrateController(backend, normalized);
  try {
    await handle.wake();
    return handle;
  } catch (error) {
    try { handle.stop(); } catch { /* partial fallback */ }
    throw error;
  }
}
