...
import { selectAssessmentV2 } from './assessment-v2-controller.mjs';
...
const eligible = row => row && (row.outcome === 'incorrect' || row.outcome === 'correct' && row.flagged);
...
export function resolveAssessmentQuestionSource({ record, plan, results = [], learning = { followups: [], suppressions: [], scheduling: 'not-computed' },
  ...
  for (const followup of record.assessmentLearning?.followups || []) {
    ...
    if (followup.actions.some(action => relevant(action) && followup.evidence.some(row =>
      row.id === action.evidenceId && matchesItem(row.item, plan.item, true) && eligible(row)))) return source;
  }
  ...
  for (const projected of received.followups) {
    ...
    if (!resultItem || !eligible({ outcome: resultItem.result, flagged: resultItem.flagged })) continue;
