...
function saveStore() {
  try {
    localStorage.setItem(STORE_KEY, JSON.stringify({ taken: S.taken, lists: S.lists, srs: S.srs }));
  } catch {
...
function back() {
  ...
  if (S.view === 'reader' || S.view === 'tray' || S.view === 'grammar' || S.view === 'levels' || S.view === 'ai') {
    S.view = 'shelf';
    render();
