...
function back() {
  ...
  if (S.view === 'reader' || S.view === 'tray' || S.view === 'grammar' || S.view === 'levels' || S.view === 'ai' || S.view === 'lessons' || S.view === 'thesaurus' || S.view === 'airead') {
    S.view = 'shelf';
    render();
...
/* --------------------------------------------------------------- lessons
 * The first Duolingo-class lane: small lessons cut from the JLPT levels the
 * dictionary already carries. Each lesson teaches its words plainly, quizzes
 * them, and on completion the words land in the memorize list — capture that
 * becomes cards without labor, feeding the same FSRS sessions as everything
 * else. One shared learner model; no separate notion of "known". */
const LESSON_SIZE = 10;
function lessonPlan(lvl) {
  const ids = laneMembers().jlpt[lvl] || [];
  const out = [];
  for (let i = 0; i < ids.length; i += LESSON_SIZE) {
    const words = ids.slice(i, i + LESSON_SIZE);
    if (words.length < 4) break;
    out.push({ id: `${lvl}-${out.length + 1}`, lvl, words });
  }
  return out;
}
...
function lessonChoices(run) {
  ...
  const right = lookup(word)?.m?.[0] || '';
  const pool = laneMembers().jlpt[run.lvl].filter((w) => w !== word);
  const wrong = new Set();
  while (wrong.size < 3 && wrong.size < pool.length) {
    const cand = lookup(pool[Math.floor(Math.random() * pool.length)])?.m?.[0];
...
function renderLessons(main) {
  ...
  if (!run) {
    ...
    for (const lvl of JLPT_LANES) {
      const plan = lessonPlan(lvl);
      if (!plan.length) continue;
      const done = plan.filter((l) => S.lessonsDone[l.id]).length;
      const head = el('p', 'eyebrow list-head');
      head.append(document.createTextNode(`${lvl} — ${done} / ${plan.length}`));
  ...
  if (run.phase === 'learn') {
    ...
    if (rec?.r) face.append(el('div', 'review-reading', rec.r));
    for (const m of (rec?.m || []).slice(0, 3)) face.append(el('div', 'review-sense', m));
    const door = biLabel('button', 'chip', 'ページへ', 'full entry');
    door.type = 'button';
    door.addEventListener('click', () => go({ t: 'word', id: word }));
    face.append(door);
    main.append(face);
  ...
  if (run.phase === 'quiz') {
    ...
    const face = el('div', 'review-face');
    face.append(el('div', 'review-front', word));
    if (run.picked != null && rec?.r) face.append(el('div', 'review-reading', rec.r));
    main.append(face);
      ...
      btn.addEventListener('click', () => {
        if (run.ix + 1 < run.words.length) { run.ix += 1; run.picked = null; run.choices = null; }
          ...
          for (const w of run.words)
            if (!takenSet.has(srsKey('word', w)))
              S.taken.push({ t: 'word', id: w, label: w, kind: NODE_KIND.word[0], kindEn: NODE_KIND.word[1], from: null, ts: Date.now() });
          saveStore();
          run.phase = 'end';
        }
        render();
