...
async function fixture(mode) {
  ...
  await context.route('**/*', async (route) => {
    ...
    if (url.pathname.endsWith('/audio/manifest.json')) {
      await ready;
      if (mode === 'delayed-fail') return route.fulfill({ status: 404, body: '' });
  ...
  const f = { context, page, errors, release };
  try {
...
async function check(name, mode, body, { learnerRecord = 'untouched' } = {}) {
  ...
  try {
    ...
    results.push({ name, pass: false, stage: f && !error.fixture ? 'body' : 'fixture', error: error.message, stack: error.stack,
      pageErrors: f?.errors || [], observed: f ? await counts(f.page).catch((e) => ({ unreadable: e.message })) : null });
    console.error(` FAIL ${name}: ${error.message}`);
    if (f) await f.page.screenshot({ path: resolve(out, `${name}.png`) }).catch(() => {});
  } finally {
    f?.release();
...
try {
  ...
  await check('first-clip-ends-second-clip-errors-stops-with-a-reason', 'recorded', async ({ page }) => {
    ...
    await page.waitForFunction(() => document.querySelector('#listen-toggle')?.getAttribute('aria-pressed') === 'false', null, { timeout: 5000 });
    const after = await counts(page);
    assert.equal(after.clips.length, 2, 'the read stops at the failed sentence');
  ...
  await check('an-invalid-stored-voice-counts-as-no-choice', 'invalid', async ({ page }) => {
    ...
    assert.match((await counts(page)).note, /recorded only in Koharune Ami|小春音アミ（仮の声・検収前）の収録だけ/u);
  ...
  await check('pending-card-audio-is-retired-by-grade-and-undo', 'card-held', async ({ page, release }) => {
  ...
  await check('pending-card-audio-is-retired-by-leaving-review', 'card-held', async ({ page, release }) => {
    await page.locator('#card-say').click();
    await page.goBack();
  ...
  await check('playing-card-audio-stops-on-grade-and-late-events-leave-the-next-card-alone', 'card', async ({ page }) => {
    ...
    await page.evaluate(() => { const old = window.__playbackFixture.clips[0]; old.onended?.(); old.onerror?.(); });
    await settle(page);
