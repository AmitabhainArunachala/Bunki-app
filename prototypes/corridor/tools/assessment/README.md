...
Admission constraints are level-keyed in `REVIEWED_WRITTEN` (`REVIEWED_N2_WRITTEN` remains an alias). `--level N1|N2` selects a pin and defaults to N2. An unknown level, or a level whose `id`/`sha256`/`bytesSha256` are still null, fails before any publication step; there is no fallback pin. The N1 pin is unfilled until its exact mapped form has independent collector evidence and John's editorial acceptance. The host policy passed with `--config` must name the pin's author family in `authorFamilyIds`.

A written-only form is prepared from a manuscript with `node prototypes/corridor/tools/assessment/prepare-originals.mjs --level N1 --manuscript file.mjs --out dir`. It evaluates exactly the hashed manuscript bytes and writes `intent.json`, `audio-scripts.json`, `authoring-notes.json` (author doubts, kept out of the learner form), `form.json` and `binding.json`. The binding records the manuscript SHA, author family, notes SHA, every item/passage mapping and the resulting form digests. The prepare step asserts that prompts, option order, key, rationale and passages survive the mapper. N1 has three sections and one 19-minute timing block. That time is an author-selected practice allocation (`authoring-rule`), not official timing. Orthography is excluded, and the declared compact tasks keep their own IDs under `practice-n1:` subjects, never official labels. The output is unreviewed authoring material. Run without arguments, the command still prepares only N2 short/medium and refuses to overwrite a revised listening form.
