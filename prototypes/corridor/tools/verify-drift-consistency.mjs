...
const aimWord = async (label, requiredClass = null, requiredGlossed = null) => {
  ...
  for (let attempt = 0; attempt < 6; attempt++) {
    last = await page.evaluate(`(() => {
      ...
      const matchingLabel = [...document.querySelectorAll('#drift-layer .word')]
      ...
      for (const el of candidates) {
        ...
        const visible = r.width > 0 && r.height > 0 && x >= 0 && x < innerWidth && y >= 0 && y < innerHeight;
        const interactive =
          style.display !== 'none' &&
      ...
      if (!matchingLabel.length) return { exists: false, aimable: false, reason: 'not found' };
      return {
        exists: true,
        aimable: false,
        reason: candidates.length ? 'not safely hit-owned' : 'required state not found',
      };
    })()`);
    if (last.aimable) {
