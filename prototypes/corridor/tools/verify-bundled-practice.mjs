...
const sizes = process.env.KAIRO_BUNDLED_PRACTICE_VIEWPORT === 'desktop' ? [1440] : [1440, 390];
...
for (const engine of engines) for (const width of sizes) {
  ...
  try {
    ...
    await page.locator('[data-sentence-teacher-response]').click(); await page.locator('#chat-input').waitFor();
    ...
    const backup = await pendingDownload, backupPath = join(out, 'bundled-practice-backup.json'); await backup.saveAs(backupPath);
...
const receipt = { version: 1, startedAt, finishedAt: new Date().toISOString(), artifactSha256: manifest.artifactSha256,
  ...
  qualification: 'Ordinary controls on bundled authored text and root-authored writing in headless persistent profiles. Fault-free connected segment plus restarts, UI restore and worker offline. No provider response, native GUI, physical device or complete learner acceptance.',
