...
const PLACEMENT: readonly {
  ...
  { level: "N5", sentence: "毎朝、日本語を勉強します。", meaning: "I study Japanese every morning." },
  { level: "N4", sentence: "歩きながら、昨日の動画をもう一度聞いてみた。", meaning: "While walking, I tried listening to yesterday’s video again." },
  { level: "N3", sentence: "文脈ごと覚えることで、自然に使えるようになる。", meaning: "Learning the whole context helps it become naturally usable." },
  { level: "N2", sentence: "内容は理解できたものの、細かなニュアンスまでは捉えきれなかった。", meaning: "Although I understood the content, I could not fully grasp the finer nuances." },
  { level: "N1", sentence: "知識の蓄積は、必ずしも運用能力の向上を意味するわけではない。", meaning: "Accumulating knowledge does not necessarily mean improving usable ability." },
] as const;
...
const downloadText = (name: string, text: string, type = "application/json"): void => {
...
export function BunkiPhase2(): ReactNode {
  ...
  const [readerWordDepth, setReaderWordDepth] =
    useState<ReaderWordDepth>(0);
  const [readerSentence, setReaderSentence] = useState<number | null>(null);
  const [immerseMode, setImmerseMode] =
    useState<ImmerseMode>("discover");
  const [readingLength, setReadingLength] =
    useState<ReadingLength>("medium");
  ...
  const [liveReadings, setLiveReadings] = useState<
    readonly LiveReadingItem[]
  >([]);
  const [liveSources, setLiveSources] = useState<
    readonly LiveSourceSummary[]
  >([]);
  ...
  const [syncConflict, setSyncConflict] =
    useState<LocalPhase2Conflict | null>(null);
      ...
      while (
        ...
        if (response.status === 409 && payload.state !== undefined) {
          ...
          const remoteRevision =
            payload.revision ?? serverRevision.current;
                ...
                try {
                  await writeLocalSnapshot({
                    ...
                    acknowledgedServerRevision:
                      durableConflict.remoteRevision,
                    savedAt: iso(),
    ...
    void (async () => {
      ...
      const detailedSyncScope =
        typeof detailedResponse?.payload.syncScope === "string"
          ...
          : cloudSyncScope;
      const scopeChangedDuringLoad =
        detailedSyncScope !== cloudSyncScope;
      if (cancelled) return;
      ...
      if (cloudDetailsUnavailable || scopeChangedDuringLoad) {
        ...
        dirty =
          cloudSyncScope !== null && !cloudLocalOnly
            ? true
            : local.dirty;
  ...
  const resolveConflictWithDevice = (): void => {
    ...
    if (conflict.paths.includes("cloud.MISSING_SOURCE_BODY:NO_LOCAL")) return;
    const local =
      deviceImportCandidateRef.current ?? currentStateRef.current;
    deviceImportCandidateRef.current = null;
    pendingSave.current = null;
    pendingLocalSave.current = null;
    if (saveTimer.current !== null) {
  ...
  const ensureDictionary = useCallback(async (): Promise<DictionaryIndex | null> => {
    ...
    try {
      ...
      setLanguageError(
        error instanceof Error ? error.message : "The dictionary could not be loaded.",
      );
      return null;
    } finally {
  ...
  const ensureLanguageTools = useCallback(async (): Promise<{
    ...
    try {
      const [nextDictionary, nextTokenizer] = await Promise.all([
        ...
        tokenizer === null ? loadJapaneseTokenizer() : Promise.resolve(tokenizer),
      ]);
  ...
  useEffect(() => {
    ...
    void fetch("/api/reading-feed", { cache: "no-store" })
      .then(async (response) => {
        if (!response.ok) throw new Error("Live reading shelves are unavailable.");
  ...
  useEffect(() => {
    ...
    const handler = (event: KeyboardEvent): void => {
      ...
      else if (overlayKey?.endsWith(":closable"))
        setOnboardingStep(0);
    };
  ...
  useEffect(() => {
    if (view !== "immerse" || activeSourceId === null || activeSourceText.trim() === "") return;
  ...
  const visibleSentences = useMemo(
    () => sentences.slice(readerWindowStart, readerWindowStart + READER_WINDOW_SIZE),
          ...
          availableLiveSources.map((source) => [
            source.domain,
            {
              ...
              articleCount: availableLiveSources
                ...
                .reduce((total, candidate) => total + candidate.articleCount, 0),
            },
          ]),
        ).values(),
  ...
  const todayReviewMinutes = Math.max(
    0,
    Math.ceil(todayReviewCount * 0.42),
  );
  ...
  const finishOnboarding = (): void => {
    update((previous) => {
      ...
      return {
        ...
        sources:
          beginner && !alreadyHasStarter ? [starter, ...previous.sources] : previous.sources,
        activeSourceId:
  ...
  const performNextAction = (): void => {
    ...
    if (nextAction.sourceId) {
      ...
      if (nextAction.kind === "analyze-source")
        window.setTimeout(() => void analyzeActiveSource(nextAction.sourceId), 0);
      return;
    }
  ...
  const analyzeActiveSource = async (sourceId = activeSource?.id ?? null): Promise<void> => {
    if (sourceId === null) return;
    const source = currentStateRef.current.sources.find((item) => item.id === sourceId);
    ...
    if (!tools) return;
    const latest = currentStateRef.current.sources.find((item) => item.id === sourceId);
    ...
    while (cursor < spans.length) {
      ...
      if (!stillCurrent || textFingerprint(stillCurrent.text) !== sourceFingerprint)
        return;
    }
    const analysisState = currentStateRef.current;
    const analysisSource = analysisState.sources.find((item) => item.id === sourceId);
    if (!analysisSource || textFingerprint(analysisSource.text) !== sourceFingerprint)
      return;
  ...
  const saveCapture = (event: FormEvent): void => {
    ...
    const kind =
      /(?:youtube\.com|youtu\.be)/i.test(url) ? "youtube" : captureKind;
  ...
  const fetchCaptions = async (): Promise<void> => {
    ...
    try {
      ...
      if (!response.ok || !payload.text) throw new Error(payload.error ?? "Captions unavailable.");
      setCaptureText(payload.text);
      setCaptureSegments(payload.segments ?? []);
      if (captureTitle.trim() === "" && payload.title) setCaptureTitle(payload.title);
  ...
  const loadTextFile = async (file: File | undefined): Promise<void> => {
    ...
    if (!/\.(txt|srt|vtt|md)$/i.test(file.name)) {
      setCaptureError("Use a .txt, .srt, .vtt, or .md export. PDF and image OCR are not connected yet.");
      return;
    }
  ...
  const importCaptionsForSource = async (source: SourceItem): Promise<void> => {
    ...
    try {
      ...
      if (payload.text.length > MAX_SOURCE_CHARACTERS)
        throw new Error("The caption track is too large for one source thread.");
      update((previous) => ({
        ...previous,
        sources: previous.sources.map((item) =>
          item.id === source.id
  ...
  const markSentenceUnderstood = (sentenceIndex: number): void => {
    ...
    update((previous) => {
      ...
      for (const token of sentenceTokens) {
        if (!token.entry || token.ambiguous || seen.has(token.entry.id)) continue;
  ...
  const admitPacket = (): void => {
    ...
    update((previous) => {
      ...
      for (const proposal of selected) {
        memories[proposal.conceptId] = recordEvidence(memories[proposal.conceptId], {
  ...
  async function rateReview(rating: 1 | 2 | 3 | 4): Promise<void> {
    ...
    const outcome = rating === 1 ? "failure" : rating === 2 ? "partial" : "success";
  ...
  const undoReview = (): void => {
    ...
    update((previous) => {
      const memories = { ...previous.memories };
      if (event.previousMemory) memories[event.conceptId] = event.previousMemory;
      ...
      return {
        ...
        memories,
        reviewEvents: previous.reviewEvents.filter((item) => item.id !== event.id),
        reviewSession:
          previous.reviewSession === null
            ? null
            : {
                ...previous.reviewSession,
                completedAt: null,
                completedCardIds: previous.reviewSession.completedCardIds.filter(
                  (id) => id !== event.cardId,
                ),
              },
        lastUndo: null,
  ...
  const deleteCard = (card: LearningCard): void => {
    if (!window.confirm("Delete this card? Its review history will remain in the activity record."))
      return;
    update((previous) => ({
      ...previous,
      cards: previous.cards.filter((item) => item.id !== card.id),
      reviewSession: removeCardFromReviewSession(previous.reviewSession, card.id),
    }));
  };
  ...
  const sendTeacher = async (event?: FormEvent): Promise<void> => {
    ...
    const enrichedFrontier = frontier.slice(0, 8).map((item) => {
      ...
      const grammar =
        item.conceptId.startsWith("grammar:")
  ...
  const stageDictionaryEntry = async (
        ...
        for (const context of contextsForConcept(snapshot, entry.id, 8)) {
          ...
          const token = tokens.find(
            (item) =>
              ...
              item.entryCandidates.some((candidate) => candidate.id === entry.id),
          );
          if (!token) continue;
          occurrence = {
    ...
    const proposal: CardProposal = {
      ...
      tags: ["dictionary", sourceSentence === "" ? "generated-proposal" : "source"],
      lineage:
  ...
  const continueDrawing = (event: ReactPointerEvent<HTMLCanvasElement>): void => {
    if (!drawing) return;
    const context = event.currentTarget.getContext("2d");
    if (!context) return;
    const point = canvasPoint(event);
    ...
    <div className="p2-page p2-today">
      ...
      <Surface className="p2-next-action">
        <div className="p2-next-icon">
          {nextAction.kind === "review" ? <ListChecks /> : nextAction.kind === "capture" ? <Link2 /> : <BookOpen />}
        </div>
        <div className="p2-next-copy">
          <span>Your best next action · about {nextAction.minutes} min</span>
          <h2>{nextAction.title}</h2>
          <p>{nextAction.detail}</p>
          <small><Brain size={14} /> Why: {nextAction.reason}</small>
        </div>
        <button className="p2-button primary p2-continue" onClick={performNextAction}>
      ...
      <div className="p2-today-grid">
        <Surface className="p2-noticed">
          ...
          {frontier.length > 0 ? (
            <div className="p2-frontier-list">
              {frontier.slice(0, 4).map((item) => (
                ...
                >
                  ...
                  <div>
                    <strong>{dictionary?.byId.get(item.conceptId)?.word ?? item.conceptId.replace("grammar:", "")}</strong>
            ...
            <div className="p2-empty compact">
              <Sparkles size={24} />
              <p>Your first lookup, source, or review will give Bunki something real to notice.</p>
            </div>
          )}
        </Surface>
        ...
        <Surface className="p2-workload">
          ...
          <div className="p2-stat-row">
            ...
            <div><strong>{summary.fragile}</strong><span>fragile</span></div>
            <div><strong>{summary.established}</strong><span>established</span></div>
          </div>
          <button className="p2-button wide" onClick={() => { setView("review"); if (due.length || hasActiveReviewSession) startSession(); }}>
            {hasActiveReviewSession ? "Resume review" : "Open review"} <ChevronRight size={17} />
          </button>
        </Surface>
      </div>
      ...
      <div className="p2-thread-row">
        <Surface>
          ...
          {activeSource ? (
            <>
              <p>{activeSource.text ? activeSource.text.slice(0, 150) : "URL saved. Add text or public captions when you have time."}{activeSource.text.length > 150 ? "…" : ""}</p>
            ...
            <button className="p2-button primary" onClick={() => setCaptureOpen(true)}>
              <Plus size={17} /> Capture what you met
            </button>
          )}
        </Surface>
        <button className="p2-walking-capture" onClick={() => setCaptureOpen(true)}>
      ...
      {immerseMode === "discover" ? (
        <div className="p2-reading-room">
          ...
          <Surface className="p2-reading-chooser">
            ...
            <nav className="p2-reading-filters" aria-label="Reading source type">
              {([
                ["all", "Everything"],
          ...
          <Surface className="p2-ai-reading-maker">
            <div>
              <span><WandSparkles size={17} /> Personal reading</span>
              <h2>Generate at your exact edge</h2>
              <p>
                Bunki asks for a natural {readingLevel}, {readingLength} article.
                Generated writing is always marked and never presented as reporting.
              </p>
            </div>
            <div>
            ...
            <section className="p2-reading-section">
              ...
              {catalogReadings.length > 0 ? (
                    ...
                    return (
                      ...
                      >
                        <div className="p2-reading-card-meta">
                          <span>{item.real ? "REAL TEXT" : "BUNKI ORIGINAL"}</span>
                          <span>{item.level}</span>
                          <span>{readingMinutes(excerpt)} min</span>
                        </div>
                        <h3>{item.title}</h3>
                        <p>{item.subtitle}</p>
                        <blockquote lang="ja">{excerpt.slice(0, 155)}…</blockquote>
                        <footer>
                ...
                <div className="p2-empty compact">
                  <BookOpen size={22} />
                  <p>
                    No built-in {readingLevel} text matches this shelf yet. Choose
                    a neighboring level or open the live shelf.
                  </p>
                </div>
              )}
            </section>
          ...
          {readingFilter === "all" || readingFilter === "live" ? (
            ...
            >
              ...
              <small className="p2-feed-note">
                ...
                Bunki imports the full page when the publisher permits it and opens
                the clearly labelled publisher summary when extraction is blocked.
                It never saves an empty article.
              </small>
            </section>
          ) : null}
      ...
      >
        <aside className="p2-source-sidebar">
          <div className="p2-source-filter">
            ...
            <span>{state.sources.filter((source) => source.status === "inbox").length} waiting</span>
          </div>
          {state.sources.length === 0 ? (
            <div className="p2-empty compact"><Link2 size={22} /><p>Save a URL or paste Japanese text.</p></div>
          ) : (
            state.sources.map((source) => (
              <button
                ...
                onClick={() => {
                  update((previous) => ({ ...previous, activeSourceId: source.id }));
                  setReaderToken(null);
                  setReaderWordDepth(0);
        ...
        <main className="p2-reader-shell">
          {activeSource === null ? (
            <div className="p2-empty hero">
              ...
              <p>Save only a URL while walking, or paste an article or transcript now.</p>
              <button className="p2-button primary" onClick={() => setCaptureOpen(true)}>Capture a source</button>
            </div>
            ...
            <Surface className="p2-source-waiting">
              ...
              {safeExternalUrl(activeSource.url) ? (
                <a className="p2-button" href={safeExternalUrl(activeSource.url) ?? undefined} target="_blank" rel="noreferrer">
                  Open original <ExternalLink size={16} />
                </a>
              ) : null}
              {activeSource.kind === "youtube" && activeSource.url.trim() !== "" ? (
                <button
                  className="p2-button primary"
              ...
              <label className="p2-field">
                ...
                <textarea
                  ...
                  onBlur={(event) => updateSourceText(activeSource, event.target.value)}
                />
              </label>
            </Surface>
          ) : (
            <>
              ...
              {readerPanelOpen ? (
                <aside className="p2-reader-appearance" aria-label="Reading appearance">
                  <header>
                    <div>
                  ...
                  <fieldset>
                    ...
                    <div className="p2-tone-options">
                      {([
                        ...
                        <button
                          key={tone}
                          className={readerSettings.tone === tone ? "active" : ""}
                          onClick={() =>
                            setReaderSettings((current) => ({ ...current, tone }))
                          }
                        >
              ...
              {(
                <>
                  {sentences.length > READER_WINDOW_SIZE ? (
                    <nav className="p2-reader-window" aria-label="Transcript pages">
                      <button
                        ...
                        onClick={() => {
                          const index = Math.max(0, readerWindowStart - READER_WINDOW_SIZE);
                          setReaderSentence(index);
                          markSentence(index);
                        }}
                      >
                      ...
                      <span>
                        ...
                        {Math.min(sentences.length, readerWindowStart + READER_WINDOW_SIZE)} of{" "}
                        {sentences.length}
                      </span>
                      <button
                        disabled={
                          readerWindowStart + READER_WINDOW_SIZE >= sentences.length
                        }
                        onClick={() => {
                  ...
                  {activeSource.id === "bunki-starter-zero" ? (
                    <section className="p2-kana-chart" aria-labelledby="kana-chart-title">
                      <header>
                        <div>
                          <span className="p2-eyebrow">Sound chart · not dictionary words</span>
                          <h2 id="kana-chart-title">Hiragana and katakana</h2>
                        </div>
                        <small>Tap a row to hear it.</small>
                      </header>
                      <div>
                        {KANA_STARTER_ROWS.map(([hiragana, katakana, romaji]) => (
                          <button
                            type="button"
                            key={hiragana}
                            onClick={() =>
                              speakJapanese(
                                hiragana.replaceAll(" ", "").replaceAll("　", ""),
                                0.72,
                              )
                            }
                          >
                  ...
                  <article
                    ...
                    lang="ja"
                    style={{ fontSize: readerSettings.size, lineHeight: readerSettings.lineHeight }}
                  >
                  {visibleSentences.map((sentence) => {
                    const sentenceTokens = tokensBySentence.get(sentence.index) ?? [];
                    const active = (readerSentence ?? activeSource.readingSentence) === sentence.index;
                    return (
                      <section
                        key={sentence.id}
                          ...
                          <p>
                            {sentenceTokens.length > 0
                              ? sentenceTokens.map((token, index) => {
                                  ...
                                  const depth = selected ? readerWordDepth : 0;
                                  return (
                                    <button
                                      key={`${String(token.position)}-${String(index)}`}
                                      ...
                                      onClick={(event) => {
                                        ...
                                        advanceReaderWord(token, sentence.index);
                                      }}
                                    >
                                      {depth >= 1 ||
                                      activeSource.id === "bunki-starter-zero" ? (
              ...
              {sentences[readerAnchor] ? (
                <div className="p2-sentence-actions">
                  ...
                  <div>
                    <button onClick={() => markSentenceUnderstood(readerAnchor)}>
                      <Check size={15} /> Understood
                    </button>
                    <button onClick={() => void mineSelectedSentence(readerAnchor)}>
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      update((previous) => ({
                        ...previous,
                        sources: previous.sources.map((source) =>
                          source.id === activeSource.id ? { ...source, notes: event.target.value } : source,
                        ),
                      }))
                    }
              ...
              <div className="p2-reader-footer">
                <div>
                  {activeSource.analysis ? (
                    <>
                      <strong>{activeSource.analysis.coverage}% estimated coverage</strong>
                      <span>{activeSource.analysis.knownCount} established · {activeSource.analysis.learningCount} learning · {activeSource.analysis.unknownCount} unmeasured</span>
                    </>
                  ) : (
                    <>
                      <strong>Not analyzed against your memory yet</strong>
                      <span>Analysis creates proposals, never hidden review debt.</span>
                    </>
                  )}
                </div>
        ...
        {readerToken?.entry && readerWordDepth >= 3 ? (
          ...
          >
            <button className="p2-sheet-close" onClick={closeReaderWord} aria-label="Close word detail"><X size={18} /></button>
            ...
            <div className="p2-sheet-word">
              <small>{readerToken.ambiguous ? "ambiguous kana reading" : readerToken.entry.reading}</small>
              <h2>{readerToken.surface}</h2>
              {readerToken.ambiguous ? (
                <span>Choose the intended word below. Bunki will not learn a default.</span>
              ) : readerToken.surface !== readerToken.entry.word ? (
                <span>dictionary form · {readerToken.entry.word}</span>
              ) : null}
            </div>
            {readerToken.ambiguous ? (
              <p>Japanese kana can represent several different written words and meanings.</p>
            ) : (
              <ol>
                {readerToken.entry.meanings.slice(0, 5).map((meaning) => <li key={meaning}>{meaning}</li>)}
              </ol>
            )}
            {!readerToken.ambiguous ? (
              <div className="p2-sheet-context">
                <span>In this sentence</span>
                <p lang="ja">{sentences[readerAnchor]?.text ?? readerToken.surface}</p>
              </div>
            ) : null}
                    ...
                    <button
                      ...
                      onClick={() =>
                        setReaderToken((current) =>
                          current === null
                            ? null
                            : { ...current, entry: candidate, ambiguous: false },
                        )
                      }
                    >
                      <strong>{candidate.word}</strong>
                      <small>{candidate.pos} · {candidate.meanings[0]}</small>
                    </button>
                  ))}
              </div>
            ) : null}
            <div className="p2-sheet-meta">
              <StatusPill state={memoryState(state.memories[readerToken.entry.id])} />
              <span>{readerToken.entry.pos}</span>
              {readerToken.entry.jlpt ? <span>{readerToken.entry.jlpt}</span> : null}
            </div>
            <div className="p2-sheet-actions">
              <button disabled={readerToken.ambiguous} onClick={() => markTokenUnderstood(readerToken)}><Check size={15} />{readerToken.ambiguous ? "Choose a word first" : "Understood"}</button>
              <button
                className="memorize"
    ...
    <div className={`p2-page p2-review ${reviewCard ? "session-active" : ""}`}>
      <header className="p2-page-head">
        <div>
          ...
          <p>FSRS schedules each card; Bunki keeps the source, skill, and reason visible.</p>
        </div>
        <div className="p2-review-head-actions">
          {state.lastUndo ? <button className="p2-button subtle" onClick={undoReview}><RotateCcw size={16} /> Undo</button> : null}
          <span>{due.length} due now</span>
        </div>
      </header>

      {state.reviewSession === null || state.reviewSession.initialCount === 0 ? (
        <Surface className="p2-review-start">
          {due.length > 0 ? (
            <>
              <ListChecks size={36} />
              <h2>{due.length} cards are at their retrieval edge.</h2>
              <p>A frozen session prevents the target from moving while you review.</p>
              <button className="p2-button primary" onClick={startSession}>Start · about {Math.ceil(reviewSessionLimit(state.profile, due.length) * 0.42)} min</button>
            </>
          ) : (
            <>
              <Check size={36} />
              <h2>Nothing is due.</h2>
              <p>Read a source, admit a staged packet, or continue a conversation.</p>
              <button className="p2-button" onClick={() => setView("immerse")}>Continue immersion</button>
            </>
          )}
        </Surface>
      ) : reviewCard === null ? (
        <Surface className="p2-review-finished">
          ...
          <p>Bunki updated skill evidence separately; one Easy answer did not magically make every dimension “known.”</p>
          <button
            className="p2-button primary"
            onClick={() => {
        ...
        <div className="p2-review-stage">
          <div className="p2-session-progress">
            ...
            <small>{reviewCard.skill} · {reviewCard.kind}</small>
          </div>
          <Surface className={`p2-review-card ${reviewRevealed ? "revealed" : ""}`}>
            <button
            ...
            {!reviewRevealed ? (
              <button className="p2-button primary p2-show-answer" onClick={() => setReviewRevealed(true)}>
              ...
              <>
                <div className="p2-card-answer" lang="ja">
                  <p>{reviewCard.back}</p>
                  {reviewCard.note ? <aside><Link2 size={14} />{reviewCard.note}</aside> : null}
                </div>
                <div className="p2-ratings">
                  {([
                    ...
                    <button key={rating} onClick={() => void rateReview(rating)}>
                      <kbd>{rating}</kbd><strong>{label}</strong><span>{intervalPreviews?.[rating]}</span>
                    </button>
                  ))}
                </div>
              </>
    ...
    <div className="p2-page p2-coach">
      <header className="p2-page-head">
        <div>
          ...
          <p>Live AI uses a bounded learner snapshot; every card remains an editable proposal.</p>
        </div>
      ...
      <div className="p2-coach-layout">
        <Surface className="p2-conversation">
          <div className="p2-mode-picker">
            {(["conversation", "correct", "explain", "roleplay"] as const).map((mode) => (
              <button key={mode} className={teacherMode === mode ? "active" : ""} onClick={() => setTeacherMode(mode)}>{mode}</button>
            ))}
          </div>
          <div className="p2-messages" aria-live="polite">
            {state.messages.length === 0 ? (
              <div className="p2-coach-welcome">
                <Bot size={31} />
                <h2>何について話しましょうか。</h2>
                <p>Talk about today’s walk, a source, philosophy, AI, or a phrase you could not quite use.</p>
                <div>
                  {["今日聞いた動画について", "最近の日本語学習", "「文脈」のニュアンス"].map((prompt) => (
                    <button key={prompt} onClick={() => setTeacherInput(prompt)}>{prompt}</button>
                  ))}
                </div>
              </div>
            ) : (
              state.messages.map((message) => (
                <article key={message.id} className={`p2-message ${message.role}`}>
                  <header>
                    <span>{message.role === "learner" ? state.profile.displayName : "Bunki"}</span>
                    <time>{new Date(message.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time>
                  </header>
                  <p lang="ja">{message.text}</p>
                  {message.supportEn ? <small>{message.supportEn}</small> : null}
                  {message.corrections.length > 0 ? (
                    <div className="p2-corrections">
                      {message.corrections.map((correction) => (
                        <div key={`${correction.original}-${correction.corrected}`}>
                          <span>{correction.original} → <strong>{correction.corrected}</strong></span>
                          <small>{correction.explanation}</small>
                        </div>
                      ))}
                    </div>
                  ) : null}
                  {message.whyThisFits ? <aside><Brain size={14} />{message.whyThisFits}</aside> : null}
                  {message.branches.length > 0 ? (
                    <div className="p2-message-branches">
                      {message.branches.map((branch) => <button key={branch} onClick={() => setTeacherInput(branch)}>{branch}</button>)}
                    </div>
                  ) : null}
                </article>
              ))
            )}
            {teacherBusy ? <div className="p2-teacher-thinking"><LoaderCircle className="spin" /> Bunki is listening to the thread…</div> : null}
            <div ref={messagesEndRef} aria-hidden="true" />
          </div>
          {teacherConnection === "signin" ? (
            <a className="p2-signin-note" href="/signin-with-chatgpt?return_to=%2F">
              Sign in with ChatGPT to use the live teacher. The local coach remains available.
            </a>
          ) : teacherConnection === "not-connected" ? (
            <div className="p2-signin-note">
              The live route is implemented, but no production OpenAI secret is configured. Replies below are clearly labelled local practice—not AI evaluation.
            </div>
          ) : null}
          <form className="p2-teacher-composer" onSubmit={(event) => void sendTeacher(event)}>
            <textarea
              ...
              maxLength={4_000}
            />
            <button disabled={teacherBusy || teacherInput.trim() === ""} aria-label="Send to coach">
              {teacherBusy ? <LoaderCircle className="spin" size={18} /> : <ArrowRight size={19} />}
            </button>
          </form>
        </Surface>
        ...
        <aside className="p2-coach-context">
          <Surface>
            ...
            <p>{activeSource?.text.slice(0, 180) || "Open an immersion source to ground this conversation."}</p>
          </Surface>
          <Surface>
  ...
  const renderDictionary = (): ReactNode => (
    <div className="p2-library-content">
      <div className="p2-library-search">
        ...
        />
        {languageBusy ? <LoaderCircle className="spin" size={18} /> : <kbd>⌘ K</kbd>}
      </div>
      {libraryQuery.trim() === "" ? (
        <div className="p2-empty hero"><Languages size={34} /><h2>Look up a word without losing its history.</h2><p>Search channels keep English, acronyms, romaji, kana, and Japanese spelling distinct.</p></div>
      ) : (
        <div className="p2-dictionary-layout">
          <div className="p2-result-list">
            {dictionaryResults.map((result) => (
              <button key={result.entry.id} className={effectiveSelectedWordId === result.entry.id ? "active" : ""} onClick={() => setSelectedWordId(result.entry.id)}>
                <div>
                  <strong>{result.entry.word}</strong>
                  <span>{result.entry.reading}</span>
                </div>
                <p>{result.entry.meanings[0]}</p>
                <small>{result.reason}{result.entry.jlpt ? ` · ${result.entry.jlpt}` : ""}</small>
              </button>
            ))}
            {dictionaryResults.length === 0 && !languageBusy ? <div className="p2-empty compact"><p>No matching entry.</p></div> : null}
          </div>
          <DictionaryDetail
            entry={selectedWord}
            state={state}
            ...
            onAsk={(entry) => { setTeacherInput(`「${entry.word}」の意味、ニュアンス、自然な使い分けを説明してください。`); setView("coach"); }}
          />
        </div>
      )}
    </div>
  ...
  const stageKanjiCard = (): void => {
    ...
    let sourceContext:
      | { readonly source: SourceItem; readonly sentence: string }
      | null = null;
    ...
    const proposal: CardProposal = {
      ...
      tags: [
        "kanji",
        "writing",
        ...(sourceContext === null ? [] : [`source:${sourceContext.source.id}`]),
      ],
      lineage:
        sourceContext === null
          ? []
  ...
  const stageKanjiRecognitionCard = (
    requestedContext?: {
      readonly source: SourceItem;
      readonly sentence: string;
    },
      ...
      dictionary && selectedKanji
        ? [
            ...
            if (leftMemory && rightMemory) {
              const recent = rightMemory.lastSeenAt.localeCompare(leftMemory.lastSeenAt);
    ...
    return (
      <div className="p2-kanji-page">
        ...
        <div className="p2-library-search compact-search">
          <Search size={18} />
          <input aria-label="Kanji lookup" value={kanjiQuery} onChange={(event) => setKanjiQuery(event.target.value)} maxLength={4} placeholder="Enter one kanji" />
        </div>
        {selectedKanji ? (
          <div className="p2-kanji-grid">
            <Surface className="p2-kanji-main">
                ...
                <div>
                  ...
                  <span>
                    ...
                    {selectedKanji.strokes.length !== selectedKanji.strokeCount
                      ...
                      : ""}
                    {" · "}grade {selectedKanji.grade ?? "—"} · {selectedKanji.jlpt ?? "unmapped"}
                  </span>
                </div>
              </div>
              <div className="p2-kanji-readings">
                <div><span>Meaning</span><strong>{selectedKanji.meanings.join(" · ")}</strong></div>
                <div><span>On</span><strong>{selectedKanji.onyomi.join("、") || "—"}</strong></div>
                <div><span>Kun</span><strong>{selectedKanji.kunyomi.join("、") || "—"}</strong></div>
              </div>
              <p className="p2-honesty-note"><ShieldCheck size={14} /> School-grade mappings can support KanKen 10–5 orientation. This Jōyō dataset does not claim KanKen 1 completeness.</p>
              <div className="p2-kanji-actions">
                ...
                <StatusPill state={memoryState(state.memories[`kanji:${selectedKanji.char}`])} />
              </div>
            </Surface>
            <Surface className="p2-handwriting">
              <div className="p2-section-title"><div><span className="p2-eyebrow">Writing self-check</span><h3>Trace, then compare</h3></div><PenLine size={20} /></div>
              <div className="p2-canvas-wrap">
                <span className={drawingEmpty ? "" : "faint"}>{selectedKanji.char}</span>
                <canvas
                  ref={canvasRef}
                  role="img"
              ...
              <button className="p2-button subtle" onClick={clearDrawing}><RotateCcw size={15} /> Clear</button>
              <small>This is a visual self-check, not handwriting recognition.</small>
            </Surface>
            <Surface className="p2-kanji-words">
              <div className="p2-section-title"><div><span className="p2-eyebrow">Vocabulary</span><h3>Encountered first</h3></div><BookMarked size={20} /></div>
              <div>
                {compounds.slice(0, 24).map((entry) => (
                  <button key={entry.id} onClick={() => { setLibraryQuery(entry.word); setSelectedWordId(entry.id); setLibraryTab("dictionary"); setKanjiFocusMode(false); setKanjiReturnContext(null); }}>
                    <strong>{entry.word}</strong><span>{entry.reading}</span><small>{entry.meanings[0]}</small><StatusPill state={memoryState(state.memories[entry.id])} />
                  </button>
                ))}
              </div>
            </Surface>
            {sourceExamples.length > 0 ? (
              <Surface className="p2-kanji-examples">
                <div className="p2-section-title">
                  <div>
                    <span className="p2-eyebrow">Examples from your immersion</span>
        ...
        ) : <div className="p2-empty hero"><PenLine size={34} /><h2>Enter one kanji.</h2></div>}
      </div>
    );
  };
  ...
  const renderGrammar = (): ReactNode => (
    <div className="p2-grammar-page">
      <div className="p2-library-search">
        <Search size={18} />
        <input aria-label="Grammar search" value={grammarQuery} onChange={(event) => setGrammarQuery(event.target.value)} placeholder="Pattern, meaning, formation, or level…" />
        <span>{GRAMMAR_PATTERNS.length} practical patterns</span>
      </div>
      <p className="p2-honesty-note"><ShieldCheck size={14} /> JLPT does not publish a canonical grammar inventory. Levels here are editorial estimates, not a completeness claim.</p>
      <div className="p2-grammar-grid">
        {grammarResults.map((pattern) => (
          <Surface key={pattern.id} className="p2-grammar-card">
            <header><span>{pattern.level}</span><StatusPill state={memoryState(state.memories[`grammar:${pattern.id}`])} /></header>
            <h3>{pattern.pattern}</h3>
            <strong>{pattern.meaning}</strong>
            <p>{pattern.formation}</p>
            <blockquote lang="ja">{pattern.example}<small>{pattern.translation}</small></blockquote>
            {pattern.contrast ? <aside><b>Contrast</b>{pattern.contrast}</aside> : null}
            {pattern.pitfall ? <aside><b>Watch</b>{pattern.pitfall}</aside> : null}
            <button className="p2-text-link" onClick={() => { setTeacherInput(`${pattern.pattern}を使った自然な会話練習をしてください。`); setView("coach"); }}>Practice with coach <ArrowRight size={14} /></button>
          </Surface>
        ))}
      </div>
    </div>
  ...
  const renderMemory = (): ReactNode => (
    <div className="p2-memory-page">
      <div className="p2-memory-overview">
        <Surface><strong>{Object.keys(state.memories).length}</strong><span>measured concepts</span></Surface>
        <Surface><strong>{summary.established}</strong><span>established across evidence</span></Surface>
        <Surface><strong>{summary.fragile}</strong><span>fragile threads</span></Surface>
        <Surface><strong>{state.sources.length}</strong><span>source lineages</span></Surface>
      </div>
      ...
      <Surface className="p2-memory-table">
        ...
        {Object.values(state.memories)
          ...
          .map((memory) => {
            ...
            return (
              <button className={memory.conceptId === state.selectedConceptId ? "active" : ""} key={memory.conceptId} onClick={() => { update((previous) => ({ ...previous, selectedConceptId: memory.conceptId })); if (entry) { setSelectedWordId(entry.id); setLibraryQuery(entry.word); setLibraryTab("dictionary"); } }}>
                <span><strong>{entry?.word ?? memory.conceptId.replace("grammar:", "")}</strong><small>{entry?.reading ?? memory.kind}</small></span>
                <span>{memory.reviewCount} attempts · {memory.exposures} encounters</span>
                <span>{memory.sourceIds.length} source{memory.sourceIds.length === 1 ? "" : "s"}</span>
  ...
  const renderLibrary = (): ReactNode => (
    <div className="p2-page p2-library">
      <header className="p2-page-head">
        <div><span className="p2-eyebrow">Library · 辞典</span><h1>Look up, connect, and deliberately learn.</h1><p>Dictionary volume is infrastructure. Bunki adds ranking, memory, lineage, kanji, grammar, and staged study.</p></div>
      </header>
      <nav className="p2-library-tabs">
        {([
          ...
          ["memory", "Memory", Brain],
        ] as const).map(([id, label, Icon]) => <button key={id} className={libraryTab === id ? "active" : ""} onClick={() => setLibraryTab(id)}><Icon size={17} />{label}</button>)}
      </nav>
  ...
  return (
    <main className={`p2-app ${hydrated ? "ready" : "hydrating"} ${zenMode ? "zen" : ""} ${kanjiFocusMode ? "kanji-focus" : ""}`} aria-busy={!hydrated} inert={!hydrated}>
      <aside className={`p2-sidebar ${mobileMenu ? "open" : ""}`}>
        <Phase2Brand />
        <button className="p2-mobile-close" onClick={() => setMobileMenu(false)} aria-label="Close navigation"><X size={20} /></button>
        <nav>
          {NAV.map((item) => {
            const Icon = item.icon;
            return (
              <button key={item.id} className={view === item.id ? "active" : ""} aria-current={view === item.id ? "page" : undefined} onClick={() => { closeReaderWord(); setView(item.id); if (item.id === "immerse") setImmerseMode("discover"); setMobileMenu(false); }}>
                <Icon size={19} /><span><strong>{item.label}</strong><small>{item.japanese}</small></span>{item.id === "review" && due.length > 0 ? <em>{due.length}</em> : null}
              </button>
            );
          })}
        </nav>
        <button className="p2-profile" onClick={() => setOnboardingStep(1)}>
          <span>{state.profile.currentLevel === "zero" ? "始" : state.profile.currentLevel}</span>
          <div><strong>{state.profile.displayName}</strong><small>{state.profile.target}</small></div>
          <ChevronRight size={16} />
        </button>
        <div className={`p2-sync ${syncStatus}`}>
          {syncStatus === "saving" || syncStatus === "loading" ? <LoaderCircle className="spin" size={14} /> : syncStatus === "saved" ? <Check size={14} /> : syncStatus === "conflict" ? <RefreshCw size={14} /> : syncStatus === "not-saved" ? <CircleAlert size={14} /> : <Pause size={14} />}
          <span>{syncStatus === "saved" ? "Personal state saved" : syncStatus === "saving" ? "Saving…" : syncStatus === "loading" ? "Loading your state…" : syncStatus === "conflict" ? "Sync needs your choice" : syncStatus === "not-saved" ? "Changes not safely stored" : "Saved on this device"}</span>
        </div>
      </aside>
      {mobileMenu ? (
        <button
      ...
      <section className="p2-main">
        <header className="p2-topbar">
          <button className="p2-menu" onClick={() => setMobileMenu(true)} aria-label="Open navigation" aria-expanded={mobileMenu}><Menu size={21} /></button>
          <Phase2Brand />
          <button aria-label="Look up Japanese" className="p2-command-search" onClick={() => { closeReaderWord(); setLibraryTab("dictionary"); setView("library"); }}>
            <Search size={17} /><span>Look up Japanese…</span><kbd>⌘ K</kbd>
          </button>
          <button
            ...
            <div>
              {syncNeedsReload || cloudBodyUnavailableWithoutLocal ? (
                ...
                <>
                  ...
                  <button className="p2-button" onClick={resolveConflictWithCloud}>
                    {deviceImportAvailable ? "Use account copy" : "Use cloud copy"}
                  </button>
                  <button
                    className="p2-button primary"
        ...
        {cloudBodyUnavailableWithoutLocal ? (
          <section className="p2-page">
            <Surface className="p2-empty">
              ...
              <p>
                Bunki will not open a partial library or replace it with a
                blank one. Reload after the source-body service recovers, or
                restore a verified device export.
              </p>
            </Surface>
          </section>
        ) : (
      ...
      <nav className="p2-mobile-nav" aria-label="Primary navigation">
        {NAV.map((item) => {
          const Icon = item.icon;
          return <button key={item.id} className={view === item.id ? "active" : ""} aria-current={view === item.id ? "page" : undefined} onClick={() => { closeReaderWord(); setView(item.id); if (item.id === "immerse") setImmerseMode("discover"); }}><Icon size={20} /><span>{item.label}</span>{item.id === "review" && due.length > 0 ? <em>{due.length}</em> : null}</button>;
        })}
      </nav>
      ...
      {!state.profile.onboardingComplete || onboardingStep > 0 ? (
        <Onboarding
          ...
          onProfile={(profile) => update((previous) => ({ ...previous, profile }))}
      ...
      {captureOpen ? (
        <div className="p2-modal-backdrop" onMouseDown={() => setCaptureOpen(false)}>
          <form className="p2-modal p2-capture-modal" onSubmit={saveCapture} onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="capture-title">
            <header><div><span className="p2-eyebrow">Walking capture</span><h2 id="capture-title">Save now. Process later.</h2></div><button type="button" onClick={() => setCaptureOpen(false)} aria-label="Close capture"><X size={20} /></button></header>
            <p>The fast path is one pasted URL and Save. Nothing enters SRS until you approve a packet.</p>
            <label className="p2-field"><span>URL</span><input autoFocus value={captureUrl} onChange={(event) => { setCaptureUrl(event.target.value); if (/(?:youtube\.com|youtu\.be)/i.test(event.target.value)) setCaptureKind("youtube"); }} placeholder="https://youtube.com/…" /></label>
            <label className="p2-field"><span>Optional title</span><input value={captureTitle} onChange={(event) => setCaptureTitle(event.target.value)} placeholder="Today’s walk · AI philosophy video" /></label>
            <button type="button" className="p2-disclosure" onClick={() => setCaptureAdvanced((current) => !current)} aria-expanded={captureAdvanced}><ChevronDown size={16} />{captureAdvanced ? "Hide text and source options" : "Add transcript, captions, or text now"}</button>
            {captureAdvanced ? (
              <div className="p2-capture-advanced">
                <div className="p2-kind-grid">
                  {SOURCE_KINDS.map((kind) => <button type="button" key={kind.id} className={captureKind === kind.id ? "active" : ""} onClick={() => setCaptureKind(kind.id)}>{kind.label}</button>)}
                </div>
                {captureKind === "youtube" && captureUrl.trim() !== "" ? <button type="button" className="p2-button" disabled={captionBusy} onClick={() => void fetchCaptions()}>{captionBusy ? <LoaderCircle className="spin" size={16} /> : <Download size={16} />} Import public captions</button> : null}
                <label className="p2-file-button"><Upload size={16} /><span>Load .txt, .srt, .vtt, or .md</span><input type="file" accept=".txt,.srt,.vtt,.md,text/plain" onChange={(event) => void loadTextFile(event.target.files?.[0])} /></label>
                <label className="p2-field"><span>Transcript, article text, or existing OCR</span><textarea value={captureText} onChange={(event) => { setCaptureText(event.target.value); setCaptureSegments([]); }} rows={10} maxLength={MAX_SOURCE_CHARACTERS} placeholder="Timestamps are cleaned when you save…" /><small>{captureText.length.toLocaleString()} / {MAX_SOURCE_CHARACTERS.toLocaleString()} characters</small></label>
                <small className="p2-honesty-note"><ShieldCheck size={14} /> Public captions depend on YouTube availability. Automatic OCR, protected-media scraping, and audio transcription are not silently simulated.</small>
              </div>
            ) : null}
            {captureError ? <div className="p2-inline-error"><CircleAlert size={16} />{captureError}</div> : null}
            <button className="p2-button primary wide" disabled={captureUrl.trim() === "" && captureTitle.trim() === "" && captureText.trim() === ""}>{captureText.trim() === "" ? "Save to inbox" : "Save and open reader"}</button>
          </form>
        </div>
      ) : null}
      ...
      {state.stagedPacket ? (
        <PacketDrawer
          ...
          onEdit={editProposal}
          onClose={() => update((previous) => ({ ...previous, stagedPacket: null }))}
          onAdmit={admitPacket}
      ...
      {editingCard ? (
        <div className="p2-modal-backdrop" onMouseDown={() => setEditingCard(null)}>
          <form className="p2-modal" onSubmit={saveCardEdit} onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="edit-card-title">
            <header><div><span className="p2-eyebrow">Card lifecycle</span><h2 id="edit-card-title">Repair the learning object</h2></div><button type="button" onClick={() => setEditingCard(null)} aria-label="Close"><X size={20} /></button></header>
            <label className="p2-field"><span>Front</span><textarea rows={4} required maxLength={MAX_CARD_FRONT_CHARACTERS} aria-invalid={cardEditError !== null && editingCard.front.trim() === ""} value={editingCard.front} onChange={(event) => { setCardEditError(null); setEditingCard({ ...editingCard, front: event.target.value.slice(0, MAX_CARD_FRONT_CHARACTERS) }); }} /></label>
            <label className="p2-field"><span>Back</span><textarea rows={6} required maxLength={MAX_CARD_BACK_CHARACTERS} aria-invalid={cardEditError !== null && editingCard.back.trim() === ""} value={editingCard.back} onChange={(event) => { setCardEditError(null); setEditingCard({ ...editingCard, back: event.target.value.slice(0, MAX_CARD_BACK_CHARACTERS) }); }} /></label>
            <label className="p2-field"><span>Note</span><textarea rows={3} maxLength={MAX_CARD_NOTE_CHARACTERS} value={editingCard.note} onChange={(event) => { setCardEditError(null); setEditingCard({ ...editingCard, note: event.target.value.slice(0, MAX_CARD_NOTE_CHARACTERS) }); }} /></label>
            {cardEditError ? <div className="p2-inline-error" role="alert"><CircleAlert size={16} />{cardEditError}</div> : null}
            <div className="p2-modal-actions">
              <button type="button" className="p2-button" onClick={() => { suspendCard(editingCard); setEditingCard(null); }}>{editingCard.suspended ? <Play size={15} /> : <Pause size={15} />}{editingCard.suspended ? "Resume" : "Suspend"}</button>
              <button type="button" className="p2-button danger" onClick={() => { deleteCard(editingCard); setEditingCard(null); }}><Trash2 size={15} /> Delete</button>
              <button className="p2-button primary"><Check size={15} /> Save changes</button>
            </div>
          </form>
        </div>
      ) : null}
...
function Onboarding({
  ...
  return (
    <div className="p2-modal-backdrop p2-onboarding-backdrop">
      <section className="p2-onboarding" role="dialog" aria-modal="true" aria-labelledby="onboarding-title">
        <header>
          <Phase2Brand />
          <div className="p2-step-dots">{[1, 2, 3, 4].map((value) => <span key={value} className={value <= step ? "active" : ""} />)}</div>
          {canClose ? <button onClick={onClose} aria-label="Close learning edge"><X size={20} /></button> : null}
        </header>
        {step === 1 ? (
          <div className="p2-onboarding-page">
            ...
            <p>This is a starting assumption, not a claim that you “know N2.” Review evidence will replace it.</p>
            <div className="p2-level-grid">
              {LEVELS.map((level) => (
                <button key={level} className={profile.currentLevel === level ? "active" : ""} onClick={() => onLevel(level)}>
                  <strong>{level === "zero" ? "From zero" : level}</strong>
                  <span>{level === "zero" ? "Kana, translation, tiny packets" : level === "N1+" ? "Nuance, literature, KanKen stretch" : `${level} provisional edge`}</span>
                </button>
              ))}
            </div>
            <button className="p2-button primary p2-onboarding-next" onClick={() => onStep(2)}>Continue <ArrowRight size={17} /></button>
          </div>
        ) : step === 2 ? (
          <div className="p2-onboarding-page">
            <span className="p2-eyebrow">2 · Provisional placement</span>
            <h1>Choose the hardest sentence you understand comfortably.</h1>
            <p>You may also keep “from zero.” This five-sentence check changes scaffolding and packet size.</p>
            <div className="p2-placement-list">
              {PLACEMENT.map((item) => (
                <button key={item.level} className={profile.currentLevel === item.level ? "active" : ""} onClick={() => onPlacement(item.level)}>
                  <span>{item.level}</span><strong lang="ja">{item.sentence}</strong><small>{item.meaning}</small>
                </button>
              ))}
            </div>
            <div className="p2-onboarding-nav"><button className="p2-button subtle" onClick={() => onStep(1)}><ArrowLeft size={16} /> Back</button><button className="p2-button primary" onClick={() => onStep(3)}>Continue <ArrowRight size={16} /></button></div>
          </div>
        ) : step === 3 ? (
          <div className="p2-onboarding-page">
            <span className="p2-eyebrow">3 · Your Japanese world</span>
            <h1>What should the language attach itself to?</h1>
            <p>These interests help Bunki choose examples and immersion starting points.</p>
            <div className="p2-interest-grid">
              {INTERESTS.map((interest) => <button key={interest} className={profile.interests.includes(interest) ? "active" : ""} onClick={() => onInterest(interest)}><Check size={15} />{interest}</button>)}
            </div>
            <label className="p2-field"><span>Daily time · minutes</span><input type="number" min={5} max={480} value={profile.dailyMinutes} onChange={(event) => onProfile({ ...profile, dailyMinutes: Math.max(5, Math.min(480, Number(event.target.value) || 5)) })} /></label>
            <div className="p2-onboarding-nav"><button className="p2-button subtle" onClick={() => onStep(2)}><ArrowLeft size={16} /> Back</button><button className="p2-button primary" onClick={() => onStep(4)}>Build my path <ArrowRight size={16} /></button></div>
          </div>
        ) : (
          <div className="p2-onboarding-page p2-plan-preview">
            ...
            <div className="p2-plan-card">
              <div><span>Scaffolding</span><strong>{profile.currentLevel === "zero" || profile.currentLevel === "N5" ? "Furigana + English support" : profile.currentLevel === "N2" || profile.currentLevel === "N1" || profile.currentLevel === "N1+" ? "Authentic sources + nuance" : "Selective furigana + source reading"}</strong></div>
              <div><span>New concepts</span><strong>{profile.currentLevel === "zero" || profile.currentLevel === "N5" ? "Maximum 3 per packet" : profile.currentLevel === "N4" || profile.currentLevel === "N3" ? "Maximum 5 per packet" : "Maximum 7 per packet"}</strong></div>
              <div><span>Daily time</span><strong>{profile.dailyMinutes} minutes</strong></div>
              <div><span>Target</span><strong>{profile.targetLevel}</strong></div>
            </div>
            <p className="p2-honesty-note"><ShieldCheck size={14} /> This is provisional. Bunki will change it from actual encounters and reviews—not a fake percentage.</p>
            <div className="p2-onboarding-nav"><button className="p2-button subtle" onClick={() => onStep(3)}><ArrowLeft size={16} /> Back</button><button className="p2-button primary" onClick={onFinish}>Enter Bunki <Sparkles size={17} /></button></div>
          </div>
        )}
      </section>
    </div>
...
function PacketDrawer({
  ...
  readonly onEdit: (id: string, field: "front" | "back" | "note", value: string) => void;
  readonly onClose: () => void;
...
function DictionaryDetail({
  ...
  if (entry === null)
    return <div className="p2-empty compact p2-dictionary-empty"><BookMarked size={27} /><p>Select an entry to see senses, memory, source occurrences, and learning actions.</p></div>;
  ...
  return (
    <Surface className="p2-dictionary-detail">
      <div className="p2-word-heading">
        <div><span>{entry.reading}</span><h2>{entry.word}</h2>{entry.altWord ? <small>{entry.altWord}</small> : null}</div>
        <button onClick={() => speakJapanese(entry.word)} aria-label={`Play ${entry.word}`}><Volume2 size={20} /></button>
      </div>
      <div className="p2-word-meta"><span>{entry.pos}</span>{entry.jlpt ? <span>{entry.jlpt} estimate</span> : null}<StatusPill state={memoryState(memory)} /></div>
      <ol className="p2-senses">{entry.meanings.slice(0, 10).map((meaning) => <li key={meaning}>{meaning}</li>)}</ol>
      {entry.containsKanji.length > 0 ? <div className="p2-kanji-links"><span>Kanji</span>{entry.containsKanji.map((character) => <button key={character} onClick={() => onKanji(character)}>{character}</button>)}</div> : null}
      <div className="p2-uncertainties"><span>What is uncertain?</span>{["meaning", "reading", "usage", "nuance", "kanji", "pronunciation"].map((item) => <button key={item} className={memory?.uncertainties.includes(item) ? "active" : ""} onClick={() => onUncertainty(entry, item)}>{item}</button>)}</div>
      <div className="p2-source-occurrences">
        <span>Seen in your sources</span>
        {occurrences.length > 0 ? occurrences.slice(0, 5).map(({ source, sentence }) => <blockquote key={source.id}><strong>{source.title}</strong><p>{sentence}</p></blockquote>) : <p>Not yet encountered in an imported source.</p>}
      </div>
      <div className="p2-detail-actions"><button className="p2-button primary" onClick={() => void onStage(entry)}><Star size={16} /> Stage a card</button><button className="p2-button" onClick={() => onAsk(entry)}><Bot size={16} /> Ask coach</button></div>
      <p className="p2-honesty-note"><ShieldCheck size={13} /> Sense ranking uses available spelling, reading, English-match, and level evidence. This packaged corpus does not include full JMdict priority, pitch accent, or licensed native audio.</p>
    </Surface>
  );
}
...
function KanjiStrokeGlyph({ entry }: { readonly entry: KanjiEntry }) {
  return (
    ...
    >
      <g className="p2-kanji-guide"><rect x="5" y="5" width="99" height="99" /><line x1="54.5" y1="5" x2="54.5" y2="104" /><line x1="5" y1="54.5" x2="104" y2="54.5" /></g>
      <g className="p2-kanji-strokes">{entry.strokes.map((path, index) => <path key={`${entry.id}-${String(index)}`} d={path} style={{ animationDelay: `${String(index * 0.1)}s` }} />)}</g>
    </svg>
  );
}
