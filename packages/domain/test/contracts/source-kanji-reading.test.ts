...
import {
  checkSourceCloze,
  checkSourceKanjiReading,
  createSourceKanjiReadingPlan,
  createSourcePracticePlan,
  gradeSourceCloze,
  gradeSourceKanjiReading,
  observeSourceListening,
  observeSourceProduction,
  parseSourceKanjiReadingCue,
  parseSourceKanjiReadingPlan,
  parseSourcePracticePlan,
  type SourceKanjiReadingCue,
  type SourceTokenOrigin,
...
const origin: SourceTokenOrigin = {
  ...
  sourceId: 'bunki-graded-n5-morning',
...
describe('1. reading-only siblings preserve existing v1 serialization', () => {
  it('separates the reading plan and contract while preserving the original cloze bytes', () => {
    ...
    expect(reading.contracts[0]).toMatchObject({
      contractId: `${reading.id}:kanji-reading`,
      contractVersion: 1,
      targetComponentId: 'kc:窓',
    ...
    expect(reading.capture.sourceRef.locator).toBe(
      `${origin.contextRef}#token=9;sentence-utf16=0,1`,
    );
    expect(reading.capture.provenance).toMatchObject({
      license: 'unverified',
      reviewStatus: 'unreviewed',
    });
    ...
    expect(
      JSON.stringify(createSourcePracticePlan(context('cloze'), origin, ['cloze', 'production'])),
    ).toBe(before);
...
describe('2. reading identity binds exact annotation facts and occurrence', () => {
  it('is independent of event ids, time and property insertion order', () => {
    ...
    const other = createSourceKanjiReadingPlan(
      context('other', '2026-09-14T00:00:00.000Z'),
      origin,
      reordered,
    );
    ...
    expect(first.id).toBe(
      `source-kanji-reading:${origin.contextRef}:0:1:v1:` +
  ...
  it('keeps repeated glyphs and UTF-16 offsets tied to the selected occurrence', () => {
    const repeated: SourceTokenOrigin = {
      ...origin,
      text: '🚀の窓と窓。',
      start: 3,
      end: 4,
    ...
    const second = createSourceKanjiReadingPlan(
      context('second'),
      { ...repeated, start: 5, end: 6, tokenSpan: { ...repeated.tokenSpan, index: 12 } },
      cue,
      first.confirmation,
    );
    expect(second.id).not.toBe(first.id);
    expect(second.capture.sourceRef.locator).toBe(
    ...
    expect(() =>
      createSourceKanjiReadingPlan(
        context(),
        { ...repeated, tokenSpan: { ...repeated.tokenSpan, index: 12 } },
        cue,
      ),
    ).toThrow();
  });
  ...
  it('preserves raw canonically equivalent reading bytes as separate annotation identities', () => {
    ...
    const first = plan(),
      other = createSourceKanjiReadingPlan(context('other'), origin, decomposed);
...
describe('3. malformed or invented source reading facts fail closed', () => {
  it('validates a prospective cue against the exact token without an event context or confirmation', () => {
    ...
    expect(() =>
      parseSourceKanjiReadingCue(cue, { ...origin, tokenSpan: { ...origin.tokenSpan, index: 10 } }),
    ).toThrow();
    expect(() =>
      parseSourceKanjiReadingCue({ ...cue, reviewStatus: 'reviewed' }, origin),
    ).toThrow();
  });
  ...
  it.each([
    ...
    expect(() =>
      parseSourceKanjiReadingCue({ ...cue, token: { ...cue.token, r } }, origin),
    ).toThrow();
    expect(() =>
      createSourceKanjiReadingPlan(context(), origin, { ...cue, token: { ...cue.token, r } }),
    ).toThrow();
  });
  ...
  it('refuses absent cue fields, invalid focus, invented review and unknown fields', () => {
    ...
    for (const altered of [
      ...
      { ...cue, focusKanji: '\ud800' },
      { ...cue, version: 2 },
      { ...cue, reviewStatus: 'reviewed' },
      { ...cue, editoriallyApproved: true },
      { ...cue, token: { ...cue.token, c: true } },
      { ...cue, token: { ...cue.token, s: ' ' } },
      { ...cue, token: { ...cue.token, s: '\ud800窓' } },
      { ...cue, token: { ...cue.token, b: '' } },
      { ...cue, token: { ...cue.token, b: '\ud800' } },
  ...
  it('requires a whole exact token, with one coordinate system and well-formed boundaries', () => {
    const compound = wholeToken('窓口'),
      compoundCue = { ...cue, token: { s: '窓口', b: '窓口', r: 'まどぐち' } };
    const { tokenSpan: _span, ...textOrigin } = origin;
    for (const altered of [
      { ...textOrigin, sentenceStart: 9 },
      { ...origin, sentenceStart: 9 },
      ...
      { ...origin, start: 0, end: 2 },
      { ...origin, start: 2, end: 1 },
    ])
  ...
  it('accepts exact bounded annotation fields without normalizing their source bytes', () => {
    ...
    const value = createSourceKanjiReadingPlan(context(), wholeToken(surface), {
      ...cue,
      token: { s: surface, b: '窓'.repeat(200), r: 'ま'.repeat(200) },
      rubySource: 'x'.repeat(160),
    });
    expect(parseSourceKanjiReadingPlan(value)).toEqual(value);
    expect(() =>
      createSourceKanjiReadingPlan(context(), wholeToken(`${surface}口`), {
        ...cue,
        token: { s: `${surface}口`, b: '窓', r: 'まど' },
      }),
    ).toThrow();
  });
  ...
  it('rejects mixed/multiple contracts, foreign events, duplicate event ids and new authority', () => {
    const reading = plan(),
      cloze = createSourcePracticePlan(context('cloze'), origin, ['cloze']);
    for (const altered of [
      { ...reading, version: 1 },
      { ...reading, kind: 'cloze' },
      { ...reading, approved: true },
      { ...reading, contracts: [] },
      { ...reading, contracts: [cloze.contracts[0]] },
      { ...reading, contracts: [...reading.contracts, ...cloze.contracts] },
      { ...reading, contracts: [reading.contracts[0], reading.contracts[0]] },
      { ...reading, capture: reading.contracts[0] },
      { ...reading, confirmation: reading.capture },
...
describe('4. deterministic reading checks and evidence preserve modality and explicit grades', () => {
  it('matches supplied kana under NFC, outer trim, ordinary full-width fold and iteration marks', () => {
    ...
    const iterated = createSourceKanjiReadingPlan(context(), wholeToken('窓'), {
      ...cue,
      token: { ...cue.token, r: 'クヽヾ' },
    });
    expect(checkSourceKanjiReading(iterated, 'くゝゞ', false).matchesReading).toBe(true);
  });

  it.each(['窓', 'mado', 'ﾏﾄﾞ', 'まどう', 'まどー', 'ま ど', 'まと', '', ' '])(
    'does not infer an equivalent reading from %j',
    (response) => {
      expect(checkSourceKanjiReading(plan(), response, false)).toMatchObject({
        matchesReading: false,
        mustRepeat: true,
      });
    },
  ...
  it.each([
    ...
    const reading = createSourceKanjiReadingPlan(context(), origin, {
      ...cue,
      token: { ...cue.token, r: supplied },
    });
  ...
  it('forces wrong and revealed answers to Again without retaining Easy confirmation', () => {
    const reading = plan();
    for (const answer of [
      { response: 'まと', revealed: false },
      { response: 'まど', revealed: true },
      { response: 'まと', revealed: true },
    ]) {
      const result = gradeSourceKanjiReading(context('grade'), reading, {
        ...input,
        ...answer,
        grade: 'easy',
      });
      ...
      expect(result.observation).toMatchObject({
        ...
        tier: 'A',
        grade: 'again',
        contractId: reading.contracts[0].contractId,
        hintsUsed: 0,
        revealedBeforeRecall: answer.revealed,
      });
  ...
  it.each(['again', 'hard', 'good', 'easy'] as const)(
    'retains an explicit %s on a matching supplied reading',
    (grade) => {
      const reading = plan();
      ...
      expect(state.gateDecisions[0]).toMatchObject({
        admitted: true,
        contractId: reading.contracts[0].contractId,
      });
    },
  ...
  it('refuses malformed responses and grade metadata', () => {
    ...
    for (const response of ['\ud800', 'ま'.repeat(1001), undefined, 1])
      expect(() =>
        Reflect.apply(checkSourceKanjiReading, null, [reading, response, false]),
      ).toThrow();
    expect(() =>
      Reflect.apply(checkSourceKanjiReading, null, [reading, 'まど', 'false']),
    ).toThrow();
    for (const change of [
      { grade: 'automatic' },
      { responseId: '' },
      ...
      expect(() =>
        Reflect.apply(gradeSourceKanjiReading, null, [context(), reading, { ...input, ...change }]),
      ).toThrow();
  });

  it('refuses reading/cloze swaps and free production or listening in the reading grader', () => {
    const reading = plan();
    const legacy = createSourcePracticePlan(
      context('legacy'),
      origin,
      ...
      {
        version: 1,
        path: 'audio/s/ami/bunki-graded-n5-morning-001.m4a',
        sha256: 'c'.repeat(64),
        bytes: 9000,
        sentenceIndex: 1,
        voice: 'ami',
...
describe('5. one original promotion supports isolated reading and cloze schedules', () => {
  it('grades only the reading contract and keeps the cloze memory-state bytes unchanged', () => {
    ...
    const reading = createSourceKanjiReadingPlan(
      context('reading', '2026-09-14T00:00:00.000Z'),
      origin,
      cue,
      cloze.confirmation,
    );
    ...
    const observed = gradeSourceKanjiReading(
      context('grade', '2026-09-14T00:01:00.000Z'),
      reading,
      input,
    ).observation;
    const after = replay([...events, observed]);
    const clozeId = cloze.contracts[0]!.contractId,
      readingId = reading.contracts[0].contractId;
    const member = (state: typeof before, id: string) =>
      state.memoryStates.find((item) => item.contractId === id);
    ...
    expect(JSON.stringify(member(after, readingId))).not.toBe(
      JSON.stringify(member(before, readingId)),
    );
    ...
    expect(after.contracts.find((contract) => contract.contractId === readingId)?.skill).toBe(
      'orthography_to_reading',
    );
    expect(after.contracts.find((contract) => contract.contractId === clozeId)?.skill).toBe(
      'discrimination',
    );
    expect(events.filter((event) => event.type === 'ThreadPromotionChanged')).toEqual([
      cloze.confirmation,
    ]);
    expect(reading.confirmation).toEqual(cloze.confirmation);
    expect(after.skippedDuplicateCount).toBe(0);
    expect(resolveComponentThread(buildTargetThreadIndex(events), 'kc:窓')).toEqual({
      linked: true,
      threadId: cloze.capture.threadId,
    });
  });
});
...
describe('6. historical facts remain parseable and corrections create new siblings', () => {
  it.each([
    ...
    { targetComponentId: 'kc:戸' },
    { promptFamilyVersion: 'source-token-reading-nfc-kana-fold@2' },
    { contractVersion: 2 },
    { cueModality: 'audio' },
    { responseModality: 'free' },
    { acceptedAnswers: ['まど', 'まと'] },
    ...
    expect(() =>
      parseSourceKanjiReadingPlan({
        ...reading,
        contracts: [{ ...reading.contracts[0], ...change }],
      }),
    ).toThrow();
  });

  it.each([
    { token: { ...cue.token, r: 'まと' } },
    { token: { ...cue.token, b: 'まど' } },
    { rubySource: 'fixture-curator' },
    { reviewStatus: 'reviewed' },
  ])('refuses altered saved cue facts under the old id: %j', (change) => {
    expect(() =>
      parseSourceKanjiReadingPlan({ ...plan(), readingCue: { ...cue, ...change } }),
    ).toThrow();
  });

  it('parses saved facts without an article and keeps old bytes when a corrected annotation is confirmed', () => {
    const original = createSourceKanjiReadingPlan(context('original'), origin, {
      ...cue,
      token: { ...cue.token, r: 'まと' },
    });
    ...
    const corrected = createSourceKanjiReadingPlan(
      context('corrected'),
      origin,
      cue,
