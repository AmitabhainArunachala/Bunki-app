...
type TranscriptData = {
  version: 1;
  provider: 'user-supplied-timed-text/v1';
  input: SuppliedTranscriptInput;
  rawSha256: string;
  body: string;
  bodySha256: string;
  cues: TranscriptCue[];
...
export type TimedTranscript = DeepReadonly<TranscriptData> & {
  readonly [verifiedTranscript]: true;
};
...
const reject = (path: string): never => {
  throw new ReadingValidationError('invalid-input', [`transcript.${path}`]);
};
...
function inputOf(raw: unknown): SuppliedTranscriptInput {
  ...
  if (
    ...
    !['file', 'paste'].includes(input.origin) ||
    !['webvtt', 'srt'].includes(input.format)
  )
    return reject('format');
  const name = prose(input.name, 255),
    text = prose(input.text, TRANSCRIPT_LIMITS.characters);
  if (!name.trim() || /[/\\]/u.test(name) || !text.trim()) return reject('empty');
  return {
    origin: input.origin as SuppliedTranscriptInput['origin'],
    format: input.format as SuppliedTranscriptInput['format'],
    name,
    text,
  };
}
...
export function importSuppliedTranscript(raw: unknown): TimedTranscript {
  ...
  for (const block of blocks) {
    ...
    if (
      input.format === 'webvtt' &&
      (/^NOTE(?:[\t ]|$)/u.test(lines[0]!) || ['STYLE', 'REGION'].includes(lines[0]!))
    ) {
      if (
        lines[0] !== 'NOTE' &&
    ...
    const startMs = timestamp(timing[1]!, input.format),
      endMs = timestamp(timing[2]!, input.format);
    const payload = lines.join('\n');
    if (
      endMs <= startMs ||
      (cues.length && startMs < cues[cues.length - 1]!.startMs) ||
      !payload.trim() ||
      payload.includes('-->')
    )
      return reject('cue');
    ...
    if (cues.length > TRANSCRIPT_LIMITS.cues || body.length > TRANSCRIPT_LIMITS.body)
  ...
  const result = immutable({
    ...
    provider: 'user-supplied-timed-text/v1',
    input,
    rawSha256: sha256Hex(input.text),
    body,
...
export function parseTimedTranscript(raw: unknown): TimedTranscript {
  ...
  if (
    ...
    data.cues.length !== expected.cues.length ||
    Object.getOwnPropertySymbols(data.cues).length ||
    Object.keys(Object.getOwnPropertyDescriptors(data.cues)).length !== data.cues.length + 1
  )
    return reject('cues');
  for (let i = 0; i < expected.cues.length; i++) {
    ...
    for (const key of ['id', 'startMs', 'endMs', 'start', 'end'] as const)
      if (cue[key] !== expected.cues[i]![key] || Object.is(cue[key], -0))
        return reject('mapping-mismatch');
  }
  return expected;
}
export function transcriptTimeRange(
  document: TimedTranscript,
  selection: {
    sourceDigest: string;
    start: number;
    end: number;
  ...
  const parsed = parseTimedTranscript(document),
  ...
  const cueIndexes = parsed.cues.flatMap((cue, index) =>
    cue.start < end && cue.end > start ? [index] : [],
  );
  if (!cueIndexes.length) return null;
  return immutable({
    startMs: Math.min(...cueIndexes.map((index) => parsed.cues[index]!.startMs)),
    endMs: Math.max(...cueIndexes.map((index) => parsed.cues[index]!.endMs)),
    cueIndexes,
  });
}
