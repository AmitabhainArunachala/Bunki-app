...
export const FILE_SOURCE_LIMITS = Object.freeze({
  bytes: 20 * 1024 * 1024,
  pages: 20,
  body: 120_000,
  regions: 8000,
...
export type FileTextRegion = Readonly<{
  start: number;
  end: number;
  text: string;
  confidence: number | null;
  bounds: FileTextBounds;
}>;
type Page = {
  page: number;
  width: number;
  ...
  provider: 'apple-vision' | 'apple-pdfkit-text' | 'apple-vision-pdf';
  revision: number | null;
  text: string;
  regions: FileTextRegion[];
};
...
export type FileTextObservation = DeepReadonly<Observation> & {
  readonly [mappedObservation]: true;
};
type Review = {
  version: 1;
  observation: FileTextObservation;
  reviewedPages: { page: number; text: string }[];
  body: string;
  bodySha256: string;
  pages: {
    page: number;
...
const reject = (path: string): never => {
  throw new ReadingValidationError('invalid-input', [`file-source.${path}`]);
};
function record<const K extends readonly string[]>(
  raw: unknown,
  required: K,
  optional: readonly string[] = [],
): Record<K[number], unknown> & Record<string, unknown> {
  if (
    !raw ||
    typeof raw !== 'object' ||
    Array.isArray(raw) ||
  ...
  if (
    ...
    keys.some((k) => !('value' in ds[k]!) || !ds[k]!.enumerable)
  )
    return reject('fields');
...
function list(raw: unknown, maximum: number): unknown[] {
  if (
    ...
    raw.length > maximum ||
    Object.getOwnPropertySymbols(raw).length ||
    Object.keys(Object.getOwnPropertyDescriptors(raw)).length !== raw.length + 1
  )
    return reject('list');
...
export function parseFileReference(raw: unknown): FileReference {
  ...
  const name = prose(d.name, 255),
    bytes = number(d.bytes, 1, FILE_SOURCE_LIMITS.bytes, true),
    pageCount = number(d.pageCount, 1, 10_000, true);
  if (
    !name.trim() ||
    /[/\\\t\r\n]/u.test(name) ||
    ['.', '..'].includes(name) ||
  ...
  if (
    !(d.kind === 'pdf' && d.mimeType === 'application/pdf') &&
    !(
      d.kind === 'image' &&
      pageCount === 1 &&
      typeof d.mimeType === 'string' &&
      ['image/png', 'image/jpeg', 'image/heic', 'image/heif', 'image/tiff'].includes(d.mimeType)
    )
  )
    return reject('type');
  return immutable({
    ...
    kind: d.kind as FileReference['kind'],
    pageCount,
  });
}
export function sameFileBytes(a: FileReference, b: FileReference): boolean {
  const left = parseFileReference(a),
    right = parseFileReference(b);
  return ['sha256', 'bytes', 'mimeType', 'kind', 'pageCount'].every(
    (k) => left[k as keyof FileReference] === right[k as keyof FileReference],
  );
}
function region(raw: unknown, body: string, isPDFText: boolean): FileTextRegion {
  ...
  const start = number(d.start, 0, body.length, true),
    end = number(d.end, start + 1, body.length, true),
    text = prose(d.text, FILE_SOURCE_LIMITS.body);
  if (body.slice(start, end) !== text || !text.length) return reject('region-text');
  const b = record(d.bounds, ['left', 'top', 'width', 'height']);
  const left = number(b.left, 0, 1),
    top = number(b.top, 0, 1),
    width = number(b.width, Number.MIN_VALUE, 1),
    height = number(b.height, Number.MIN_VALUE, 1);
  if (left + width > 1 + 1e-9 || top + height > 1 + 1e-9) return reject('bounds');
  const confidence =
    !Object.hasOwn(d, 'confidence') || d['confidence'] === null
      ? null
      : number(d['confidence'], 0, 1);
...
export function parseFileTextObservation(raw: unknown): FileTextObservation {
  ...
  const d = record(raw, ['version', 'file', 'pages']),
    file = parseFileReference(d.file);
  ...
  const pages = list(d.pages, FILE_SOURCE_LIMITS.pages).map((value) => {
    ...
    const page = number(p.page, 1, file.pageCount, true),
      width = number(p.width, Number.MIN_VALUE, 40_000),
      height = number(p.height, Number.MIN_VALUE, 40_000);
    const rotation = number(p.rotation, 0, 270, true),
      text = prose(p.text, FILE_SOURCE_LIMITS.body),
      pdfText = p.provider === 'apple-pdfkit-text';
    if (
      (previous && page !== previous + 1) ||
      ![0, 90, 180, 270].includes(rotation) ||
      p.unit !== 'utf16-code-unit' ||
      (file.kind === 'image'
        ? p.provider !== 'apple-vision' || rotation !== 0 || width * height > 40_000_000
        : typeof p.provider !== 'string' ||
          !['apple-pdfkit-text', 'apple-vision-pdf'].includes(p.provider)) ||
      p.coordinateSpace !==
        (pdfText ? 'normalized-top-left-unrotated-media-box' : 'normalized-top-left-oriented-page')
    )
      return reject('page');
    previous = page;
    total += text.length;
    const revision =
      !Object.hasOwn(p, 'revision') || p['revision'] === null
        ? null
        : number(p['revision'], 1, 1000, true);
    ...
    if (total > FILE_SOURCE_LIMITS.body || regionCount > FILE_SOURCE_LIMITS.regions)
    ...
    return {
      ...
      height,
      rotation,
      coordinateSpace: p.coordinateSpace as string,
      unit: 'utf16-code-unit' as const,
      provider: p.provider as Page['provider'],
      revision,
      text,
      regions,
    };
...
export function reviewFileText(observation: unknown, input: unknown): ReviewedFileText {
  const parsed = parseFileTextObservation(observation),
    values = list(input, FILE_SOURCE_LIMITS.pages);
  ...
  const pages = reviewedPages.map((p, i) => {
    ...
    const original = parsed.pages[i]!,
      changed = p.text !== original.text;
    return {
      page: p.page,
      start,
      end: body.length,
      ...
      regions: changed
        ? []
        : original.regions.map((r) => ({ ...r, start: r.start + start, end: r.end + start })),
    };
  });
...
export function parseReviewedFileText(raw: unknown): ReviewedFileText {
  ...
  if (d.version !== 1 || d.body !== expected.body || d.bodySha256 !== expected.bodySha256)
    return reject('review-mapping');
  ...
  pages.forEach((rawPage, i) => {
    const p = record(rawPage, ['page', 'start', 'end', 'changed', 'regions']),
      e = expected.pages[i]!;
    if (
      p.page !== e.page ||
      p.start !== e.start ||
      ...
      return reject('review-mapping');
    const regions = list(p.regions, 4000).map((r) =>
      region(r, expected.body, expected.observation.pages[i]!.provider === 'apple-pdfkit-text'),
    );
    if (JSON.stringify(regions) !== JSON.stringify(e.regions)) return reject('review-mapping');
  });
  return expected;
}
export function fileTextSourceRange(
  document: ReviewedFileText,
  selection: { sourceDigest: string; start: number; end: number; quote: string },
): DeepReadonly<{ pages: { page: number; changed: boolean; regions: FileTextRegion[] }[] }> | null {
  const parsed = parseReviewedFileText(document),
    { start, end, quote, sourceDigest } = selection;
  number(start, 0, parsed.body.length, true);
  number(end, start + 1, parsed.body.length, true);
  if (
    sourceDigest !== parsed.bodySha256 ||
    parsed.body.slice(start, end) !== quote ||
    !isWellFormedText(quote)
  )
