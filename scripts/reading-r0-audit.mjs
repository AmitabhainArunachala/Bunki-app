...
if (!candidate || !/^[0-9a-f]{40}$/.test(candidate) || !output) {
  console.error('usage: node scripts/reading-r0-audit.mjs --candidate <40-char-sha> --out <directory>');
  process.exit(2);
}
...
entries.push({ ...indexEntry, path: '404.html', source: 'generated copy of prototypes/corridor/index.html' });
...
if (artifactDir) {
  ...
  if (JSON.stringify(downloadedEntries) !== JSON.stringify(projected)) {
    console.error('downloaded workflow artifact is not byte-identical to the candidate source projection');
    process.exit(1);
  }
  downloadedArtifact = {
    compared: true,
...
const artifactManifest = {
  ...
  candidate,
  tree,
  scope: 'deterministic source projection of .github/workflows/pages-app.yml Assemble the Pages site step',
  evidenceClass: downloadedArtifact?.compared ? 'WORKFLOW_ARTIFACT_COMPARISON' : 'SOURCE_INSPECTION',
  downloadedArtifact,
...
const group = (rows, key) =>
  Object.fromEntries(
    [...rows.reduce((map, row) => map.set(row[key] || '(missing)', (map.get(row[key] || '(missing)') || 0) + 1), new Map())]
...
const allArticleJson = (await walk(articleRoot))
  ...
  .filter((relative) => relative.endsWith('.json') && !['index.json', 'archive-index.json'].includes(relative));
...
const runtimeEnglishTitleCount = titleBlock ? [...titleBlock[1].matchAll(/^\s*['"][^'"\n]+['"]\s*:/gm)].length : 0;
...
const corpusCensus = {
  ...
  primary: {
    ...
    metadataPresence: Object.fromEntries(fields.map((field) => [field, countPresent(primary, field)])),
    charLength: {
      min: lengths[0],
    ...
    publicApprovalReceipts: primary.filter((row) => exists(row.approvalState) || exists(row.editorial)).length,
  },
  archive: {
    ...
    metadataPresence: Object.fromEntries(fields.map((field) => [field, countPresent(archive, field)])),
  },
  all: {
    total: ids.length,
    ...
    pendingVerification: [...primary, ...archive].filter((row) => row.pendingVerification === true).length,
    publicationStatus: {
      approved: 0,
      private: 0,
      draft: 0,
...
await writeFile(path.join(output, 'artifact-manifest.json'), `${JSON.stringify(artifactManifest, null, 2)}\n`);
await writeFile(path.join(output, 'corpus-census.json'), `${JSON.stringify(corpusCensus, null, 2)}\n`);
...
await writeFile(path.join(output, 'evidence-manifest.json'), `${JSON.stringify(evidenceManifest, null, 2)}\n`);
console.log(
