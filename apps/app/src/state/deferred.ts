...
export const DEFERRED_INTEGRATION: readonly DeferredItem[] = [
  ...
  },
  {
    ...
    closesWith:
      'An ADR-002 decision covering both halves: an amendment widening uncertaintyMark to the five-value enum, and an amendment giving a post-capture mark an event family of its own — or an explicit ruling that a mark is a capture-time-only fact and everything after it stays app-local. Until then the UI collects the dimension, keeps the annotation beside the log, and says on screen which of the two cases the learner is actually in — see uncertaintyLogNote in src/state/store.ts. RENKAN R4-A closed the durability half without touching the schema: the annotation is journaled to a side-store inside the persistence seam and restored on reload, so only the event-log/export half remains open here.',
  },
  {
    id: 'WP05-D3',
