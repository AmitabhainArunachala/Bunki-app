...
const screen = (name: string): string =>
  read(resolve(APP_ROOT, "src/screens", name));
...
describe("every screen file has a recorded owner and a route", () => {
  ...
  it("renders every screen from some route file", () => {
    ...
    expect(
      unrouted,
      "each screen needs a route file under apps/app/app/",
    ).toEqual([]);
  });
});
...
describe("dictionary indices are never rendered (REQ-UI-03)", () => {
  ...
  it.each(FORBIDDEN_INDICES)("never renders a %s index", (indexName) => {
    ...
    const pattern =
      indexName === "SKIP"
        ? /\bSKIP\b/
        : new RegExp(indexName.replace(/\s+/g, "\\s+"), "i");
  ...
  it("has no index field in the seed to render in the first place", async () => {
    ...
    for (const kanji of seedDataset.kanji) {
      ...
      expect(
        fields.filter((field) =>
          /index|skip|nelson|henshall|morohashi/i.test(field),
        ),
      ).toEqual([]);
    }
  });
...
describe("the seed entry disclosure reaches the pages that need it", () => {
  ...
  it("comes from @bunki/seed rather than being retyped", () => {
    ...
    const hardcoded = sourceFiles.filter((file) =>
      read(file).includes(
        "Readings and senses in this seed are representative",
      ),
    );
...
describe("every screen implements all four REQ-UI-09 states", () => {
  it.each(SCREEN_SOURCES)(
    "%s screen defines all three panels",
    (_name, source) => {
   ...
   */
  it.each(SCREEN_SOURCES)(
    "%s screen renders no two state panels at once",
    (_name, source) => {
      ...
      for (const match of body.matchAll(/<>([\s\S]*?)<\/>/g)) {
        const inner = match[1] ?? "";
        const together = STATE_PANELS.filter((panel) =>
...
describe("architectural boundaries (controller §5)", () => {
  ...
  const importsOf = (specifier: string): RegExp =>
    new RegExp(
      String.raw`(?:from|import|require)\s*\(?\s*['"]` + specifier,
      "g",
    );
  ...
  it("re-exports no adapter class from the seam", () => {
    ...
    for (const forbidden of ["ProvisionalWebEventStore", "SqliteEventStore"]) {
      expect(seam, `${forbidden} must not leave the seam`).not.toMatch(
        new RegExp(
          String.raw`export\s+(?:type\s+)?\{[^}]*\b${forbidden}\b`,
          "s",
        ),
      );
    }
  ...
  it("is reached only from src/state/", () => {
    const importers = sourceFiles
      .filter((file) =>
        /['"][^'"]*\bpersistence\/(?:index|shared|platform-store)/.test(
          code(file),
        ),
      )
      .map(rel)
  ...
  it("never imports ts-fsrs", () => {
    const offenders = sourceFiles.filter((file) =>
      importsOf("ts-fsrs").test(code(file)),
    );
    expect(offenders.map(rel)).toEqual([]);
  });
  ...
  it("constructs events only through the kernel factory", () => {
    ...
    const stateFiles = sourceFiles.filter((file) =>
      rel(file).startsWith("src/state/"),
    );
    const factoryUsers = stateFiles.filter((file) =>
      code(file).includes("createDomainEvent"),
    );
    expect(factoryUsers.map(rel)).toEqual(["src/state/memory-store.ts"]);

    const screenFiles = sourceFiles.filter((file) =>
      rel(file).startsWith("src/screens/"),
    );
  ...
  it("holds no scheduling, grading or evidence logic", () => {
    for (const file of sourceFiles) {
      ...
      expect(source, rel(file)).not.toMatch(
        /\bstability\b|\bretrievability\b|\bmemoryState\b/i,
      );
      expect(source, rel(file)).not.toMatch(
        /\bschedule(d|r)?\b|\bdueAt\b|\bnextReview\b/i,
      );
    }
  });
});
...
describe("claim boundaries (REQ-GATE-03)", () => {
  ...
  it("does not present the in-memory store as durable", () => {
    ...
    expect(captureSource).not.toMatch(
      /saved\s+permanently|saved\s+forever|never\s+lose/i,
    );
  });
  ...
  it("derives what it says about the event log rather than asserting it", () => {
    for (const [name, source] of [
      ...
      expect(source, name).not.toMatch(
        /records that a mark exists, not which one/,
      );
    }
    expect(read(resolve(APP_ROOT, "src/state/store.ts"))).toContain(
      "markedAtCapture",
    );
  });
  ...
  it("keeps unresolved durable-write copy truthful after later activity", () => {
    ...
    expect(debugSource).toContain(
      "A newer save can succeed without resolving this earlier gap.",
    );
