...
describe('T-01 / T-16-web — a capture survives a reload of the app', () => {
  it('finds the thread again after a restart', async () => {
    ...
    expect(second.store.readAll().map((event) => event.eventId)).toEqual([ack.events[0]?.eventId]);
  });
  ...
  it('keeps promotion state across the restart', async () => {
    ...
    first.store.execute({ kind: 'promote', threadId: ack.threadId, to: 'learn' });
    await first.flush();

    const second = await launch(snapshotStore, 'run2-');
    expect(second.store.getSnapshot().threadsById[ack.threadId]?.state.promotion).toBe('learn');
  });
...
describe('what a reload honestly cannot restore', () => {
  ...
  it('keeps the fact of a mark and drops the dimension, saying which', async () => {
    ...
    expect(first.store.getSnapshot().threadsById[ack.threadId]?.uncertainty?.dimension).toBe(
      'reading',
    );
    await first.flush();
  ...
  it('re-attaches a thread to its seed entry through the injected resolver', async () => {
    ...
    expect(withResolver.store.getSnapshot().threadsById[ack.threadId]?.lexemeId).toBe('lex-bunki');

    // …and without one, the link is absent rather than invented.
    const withoutResolver = await launch(snapshotStore, 'run3-');
    expect(withoutResolver.store.getSnapshot().threadsById[ack.threadId]?.lexemeId).toBeNull();
  });
});

describe('durabilityFor', () => {
  it('refuses to promise a reload a denied browser cannot survive', () => {
    expect(durabilityFor('web-provisional', false)).toBe('in-memory-session-only');
