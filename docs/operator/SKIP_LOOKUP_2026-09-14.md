# SKIP lookup — data, engine and integrated lookup

**Date:** 2026-09-14

**Scope:** canonical `prototypes/corridor/`; isolated sidecar and dedicated lookup

**Status:** the September 14 local candidate integrates the engine, lookup UI,
reference return paths and packaging. Browser and raw-archive checks are recorded
in the [production build record](KAIRO_PRODUCTION_BUILD_2026-09-10.md). Native
delivery and product-wide acceptance remain separate checks. The local standalone
regression includes an offline learning sequence and exact record reload; it is
not the full set of persistent learner journeys.
...
`proprietary_safe`. The initial data/engine package did not modify the UI. The
combined candidate adds explicit lookup routes and the required script, style,
service-worker and standalone assets. Ordinary kanji sheets expose a code-free
shape-lookup door; numeric codes remain in the dedicated lookup surface.
...
```sh
python3 prototypes/corridor/tools/build-skip-data.py
...
```sh
python3 prototypes/corridor/tools/build-skip-data.py --archive /path/to/kanjidic2-en-3.6.2+20260803141815.json.zip
python3 prototypes/corridor/tools/build-skip-data.py --archive /path/to/kanjidic2-en-3.6.2+20260803141815.json.zip --check
...
The integrated UI loads the script and sidecar, routes only `kind: "skip"` or
`"invalid"` away from ordinary dictionary search, exposes loading/failure/empty
states and attribution, distinguishes alternates, and retains context on return.
An explicit capture of an uncatalogued kanji retains its complete observed answer
and source version in the learner record. Missing answers cannot be captured or
graded; a lookup alone creates no card. The standalone full-flow check and native
writing coverage require their own evidence.
