...
## Addendum — battery on the KAGAMI-merged head (main #86 merged in)

# Battery run — claude/bunki-kairo-pr76-review-qa1qnh @ 6e41414cedf126957fa3e5453f05ad0baee19232
...
| storage-integ | 0 | } |
| drift-fast | 0 | shots  → /home/user/Bunki-app/docs/audits/drift-consistency-shots |
| corridor-ai | 0 | 48/48 checks passed |
| native-readings | 0 | report → /home/user/Bunki-app/docs/build-evidence/kairo-feel-lock/native-readings/browser-verifica |
| replay | 0 |  |
| export | 0 |  |
| e2e-build | 0 | Exported: dist |
| e2e | 0 |   44 passed (1.1m) |
| corpus-pytest | 0 | 185 passed, 2 skipped, 29 deselected in 7.62s |

Completed: 2026-08-30T12:14:21Z

**15/16.** The one red — `corridor` (exit 2, the variants card-preview
wait at verify-corridor.mjs:1088) — is **inherited from `main`**, not
this branch's: it reproduced 3/3 on this head AND 1/1 on pristine
`main` @ 8504cea4, while an isolated fresh-state repro of the same
funnel passes. The regression arrived inside #86's range and is filed
as **issue #87** with the diagnosis (async sheet re-renders under
accumulated session state un-land the fold-head tap). Everything this
PR's diff touches ran green, including native-readings — in-battery
this time, no race. The prior receipts above stand unedited.
