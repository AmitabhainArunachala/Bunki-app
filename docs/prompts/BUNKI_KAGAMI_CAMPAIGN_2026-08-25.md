...
concept and supplied the node taxonomy: failures land on _reading_ nodes (歩→ほ),
_confusion_ edges (歩↔足), _sense_ nodes (によって), _form_ nodes (potential
...
2. **Amendment of 2026-08-24 recorded**: the sensei may create cards and write
   _observations_; it still never grades, never schedules, never writes FSRS state.
...
_Verify:_ battery; new AI probes; export→import round-trip keeps unknown-kind rows.
...
_Verify:_ battery + verify-kagami; a11y on the new view.
...
_Verify:_ battery; AI probes assert prompts carry model context; interview writes typed rows.
...
_Verify:_ battery incl. verify-drift suites; a probe that 自 with an empty ledger falls
...
_Verify:_ battery; review-room renders multi-sentence fronts; rights strings ride each card.
...
_Verify:_ verify-feed 27+ green; a fixture export steers a tranche deterministically.
...
_Verify:_ battery; probe that no output string ever claims a JLPT pass.
